    ...
    if (i_this->m05B0 == 0) {
        if (soundFileIdx >= 0) {
            i_this->mpMorf->setAnm(
                (J3DAnmTransform*)dComIfG_getObjectRes("Mo2", bckFileIdx),
                loopMode,
                morf,
                speed,
                0.0f,
                -1.0f,
                dComIfG_getObjectRes("Mo2", soundFileIdx)
            );
        } else {
...
static void ke_control(mo2_class* i_this, ke_s* param_2, int param_3) {
    ...
    if (l_mo2HIO.m020 == 0) {
        ...
        pos.y += 75.0f;
        gndChk.SetPos(&pos);
        dVar10 = (dComIfG_Bgsp()->GroundCross(&gndChk) + 2.5f);
...
static BOOL daMo2_Draw(mo2_class* i_this) {
    ...
    J3DShape* shape = model->getModelData()->getMaterialNodePointer(0x03)->getShape();
...
static BOOL daMo2_Execute(mo2_class* i_this) {
    ...
    actor->setBtAttackData(0.0f, 10.0f, 10000.0f, i_this->mParryOpeningType);
...
actor_process_profile_definition g_profile_MO2 = {
    ...
    /* Proc Name    */ fpcNm_MO2_e,
    /* Proc SubMtd  */ &g_fpcLf_Method.base,
    /* Size         */ sizeof(mo2_class),
    /* Size Other   */ 0,
    /* Parameters   */ 0,
    /* Leaf SubMtd  */ &g_fopAc_Method.base,
    /* Draw Prio    */ fpcDwPi_MO2_e,
