...
static void hand_draw(sitem_class* i_this) {
    ...
    i_this->mLine.update(10, color, &i_this->tevStr);
    dComIfGd_set3DlineMat(&i_this->mLine);
    if (i_this->mCutLength > 0.1f) {
        GXColor color = {0, 180, 50, 255};
        i_this->mCutLine.update(5, color, &i_this->tevStr);
...
static void hand_mtx_set(sitem_class* i_this) {
    ...
    mDoMtx_XrotM(*calc_mtx, i_this->mModelAngle.x);
    mDoMtx_YrotM(*calc_mtx, i_this->mModelAngle.y);
...
static void my_break(sitem_class* i_this) {
    ...
    fopAcM_createItemFromTable(&i_this->mPos, i_this->mItemTable, i_this->mItemBit & 0x7F, i_this->home.roomNo, 0, &angle, 1, NULL);
    if (i_this->mType <= 1) {
...
static BOOL daSitem_Delete(sitem_class* i_this) {
    dComIfG_resDelete(&i_this->mPhase, "Sitem");
    i_this->mSmoke[0].remove();
    i_this->mSmoke[1].remove();
    return TRUE;
}
