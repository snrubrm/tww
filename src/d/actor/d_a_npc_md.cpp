...
daNpc_Md_HIO3_c::daNpc_Md_HIO3_c() {
    ...
    m1A = 0;
    m1C = 0;
    m1E = DEMO_SELECT(3000, 3800);
    m20 = DEMO_SELECT(6000, 5800);
    m22 = 0x7FFF;
    m24 = DEMO_SELECT(5800, -0x7FFF);
}
...
cPhs_State daNpc_Md_c::create() {
    ...
    setTalkType(fopAcM_GetParam(this) >> 0x08);
    if ((int)getTalkType() == -2) { // !@bug: Comparing unsigned value with -2 is always false.
    ...
    if (phase_state == cPhs_COMPLEATE_e) {
        ...
        fopAcM_SetMtx(this, getModel()->getBaseTRMtx());
...
static BOOL waistNodeCallBack(J3DNode* node, int calcTiming) {
    if (calcTiming == J3DNodeCBCalcTiming_In) {
        ...
        if (i_this) {
            s32 jntNo = joint->getJntNo();
            MtxP r3 = model->getAnmMtx(jntNo);
            mDoMtx_multVecArray(r3, waistVecDat, i_this->getPHairWall(), ARRAY_SIZE(waistVecDat));
            mDoMtx_stack_c::copy(model->getAnmMtx(jntNo));
...
BOOL daNpc_Md_c::createHeap() {
    ...
    JUT_ASSERT(DEMO_SELECT(1978, 2013), m_backbone1_jnt_num >= 0);
    m_backbone2_jnt_num = modelData->getJointName()->getIndex("backbone2");
    JUT_ASSERT(DEMO_SELECT(1981, 2016), m_backbone2_jnt_num >= 0);
    m_armR_jnt_num = modelData->getJointName()->getIndex("armR");
    JUT_ASSERT(DEMO_SELECT(1984, 2019), m_armR_jnt_num >= 0);
    m_armL_jnt_num = modelData->getJointName()->getIndex("armL");
    JUT_ASSERT(DEMO_SELECT(1987, 2022), m_armL_jnt_num >= 0);
    m_neck_jnt_num = modelData->getJointName()->getIndex("neck");
    JUT_ASSERT(DEMO_SELECT(1990, 2025), m_neck_jnt_num >= 0);
    ...
    JUT_ASSERT(DEMO_SELECT(2028, 2065), m_armRloc_jnt_num >= 0);
    m_armLloc_jnt_num = modelData->getJointName()->getIndex("armLloc");
    JUT_ASSERT(DEMO_SELECT(2031, 2068), m_armLloc_jnt_num >= 0);
    m_handL_jnt_num = modelData->getJointName()->getIndex("handL");
    JUT_ASSERT(DEMO_SELECT(2034, 2071), m_handL_jnt_num >= 0);
    ...
    if (!isTypeShipRide()) {
        ...
        JUT_ASSERT(DEMO_SELECT(2058, 2097), m_wingRloc_jnt_num >= 0);
        m_wingLloc_jnt_num = modelData->getJointName()->getIndex("wingLloc");
        JUT_ASSERT(DEMO_SELECT(2061, 2100), m_wingLloc_jnt_num >= 0);
        m_wingR2_jnt_num = modelData->getJointName()->getIndex("wingR2");
        JUT_ASSERT(DEMO_SELECT(2064, 2103), m_wingR2_jnt_num >= 0);
        m_wingL2_jnt_num = modelData->getJointName()->getIndex("wingL2");
        JUT_ASSERT(DEMO_SELECT(2067, 2106), m_wingL2_jnt_num >= 0);
        m_wingR3_jnt_num = modelData->getJointName()->getIndex("wingR3");
        JUT_ASSERT(DEMO_SELECT(2070, 2109), m_wingR3_jnt_num >= 0);
        m_wingL3_jnt_num = modelData->getJointName()->getIndex("wingL3");
        JUT_ASSERT(DEMO_SELECT(2073, 2112), m_wingL3_jnt_num >= 0);
...
BOOL daNpc_Md_c::lightHitCheck() {
    ...
    if (lightHit) {
        ...
        if (m304C.getEmitter() == NULL) {
            m304C.makeEmitter(dPa_name::ID_AK_SN_MIRRORSHIELD00, mpHarpLightModel->getBaseTRMtx(), &current.pos, NULL);
    ...
    if (mAcch.ChkGroundHit()) {
        if (!isTypeM_Dra09()) {
            if (dComIfG_Bgsp()->GetSpecialCode(mAcch.m_gnd) == 1) {
                ...
                speedF += 0.5f;
                speedF = cLib_maxLimit<f32>(speedF, l_HIO.m008.m08);
            } else {
                speedF = 0.0f;
            }
        }
...
BOOL daNpc_Md_c::waitNpcAction(void*) {
    if (mActionStatus == ACTION_STARTING) {
        ...
        if (isTypeSea() && !dComIfGs_isEventBit(dSv_event_flag_c::UNK_1402)) {
            ...
            s16 headAngle = shape_angle.y + getHead_y() + getBackbone_y();
...
BOOL daNpc_Md_c::talkNpcAction(void*) {
    if (mActionStatus == ACTION_STARTING) {
        ...
        if (mActionStatus == ACTION_ONGOING_1) {
            ...
            if (talk(0)) {
                if ((isTypeAtorizk()) || (isTypeM_Dra09())) {
                    mActionStatus = 4;
        ...
        } else if ((isTypeM_Dra09()) && (dComIfGs_isEventBit(dSv_event_flag_c::UNK_1101))) {
            setNpcAction(&daNpc_Md_c::waitNpcAction);
        }
        mJntCtrl.setTrn();
        lookBack(1, 0, 0);
...
s16 daNpc_Md_c::windProc() {
    ...
    current.angle.y = cM_atan2s(local_88.x, local_88.z);
    speedF = local_88.absXZ();
    speedF = cLib_maxLimit<f32>(speedF, l_HIO.m13C);
    speed.y = local_88.y;
    if (m3148 == 0) {
        maxFallSpeed = l_HIO.m110;
        gravity = l_HIO.m114;
...
BOOL daNpc_Md_c::fall02NpcAction(void* param_1) {
    if (mActionStatus == ACTION_STARTING) {
        ...
        if ((isTypeM_Dai()) && (mAcch.ChkGroundHit())) {
...
BOOL daNpc_Md_c::piyo2NpcAction(void*) {
    if (mActionStatus == ACTION_STARTING) {
        ...
        int local_28 = m312C;
        if (local_28 != 0) {
            if ((((isTypeAtorizk()) || (isTypeAdanmae())) || (isTypeM_Dra09())) || (isTypeSea())) {
                cLib_onBit<u32>(attention_info.flags, fopAc_Attn_LOCKON_TALK_e | fopAc_Attn_ACTION_SPEAK_e);
            } else {
                cLib_offBit<u32>(attention_info.flags, fopAc_Attn_LOCKON_TALK_e | fopAc_Attn_ACTION_SPEAK_e);
            }
...
f32 daNpc_Md_c::checkForwardGroundY(s16 param_1) {
    ...
    //! @bug(?): is this supposed to be -G_CM3D_F_INF?
    return -10000000.0f;
}
...
BOOL daNpc_Md_c::searchNpcAction(void*) {
    ...
    if (mActionStatus == ACTION_STARTING) {
        ...
        if (mActionStatus == ACTION_ONGOING_1) {
            ...
            if (door || !temp3 ||
                (dComIfGp_checkPlayerStatus0(0, daPyStts0_UNK2000000_e | daPyStts0_HANG_e | daPyStts0_UNK1_e) || player->checkAttentionLock()))
            {
                temp = 0.0f;
                offNpcCallCommand();
            ...
            vec = current.pos;
            vec.y = eyePos.y;
            mJntCtrl.lookAtTarget(&temp4, &vec2, vec, shape_angle.y, l_HIO.mNpc.mMaxTurnStep, false);
            if (speedF < 0.001f) {
                setNpcAction(&daNpc_Md_c::waitNpcAction);
            } else {
                shape_angle.y = temp4;
...
BOOL daNpc_Md_c::escapeNpcAction(void*) {
    if (mActionStatus == ACTION_STARTING) {
        ...
        if (mActionStatus == ACTION_ONGOING_1) {
            ...
            f32 f30 = 1.0f;
            if (m313F != 0) {
                s16 sVar5 = (m313F < 0) ? DEMO_SELECT(-0x4000, (s16)-0x4000) : DEMO_SELECT(0x4000, (s16)0x4000);
                m3144 = DEMO_SELECT(sVar5, (s16)sVar5) + cM_atan2s(m30C4.x, m30C4.z);
            } else if (m3146 == 0) {
                setNpcAction(&daNpc_Md_c::waitNpcAction);
                return TRUE;
            }
            s16 local_38 = m3144;
            s16 sVar5 = local_38;
            if (routeCheck(f31, &local_38)) {
                cLib_distanceAngleS(sVar5, local_38);
            }
            walkProc(f30, local_38);
            cLib_addCalcAngleS(&shape_angle.y, current.angle.y, l_HIO.m008.m24, l_HIO.m008.m20 * 2, l_HIO.m008.m22 * 2);
            s16 temp4 = shape_angle.y;
            if (speedF < 0.001f) {
                setNpcAction(&daNpc_Md_c::waitNpcAction);
...
BOOL daNpc_Md_c::flyPlayerAction(void*) {
    ...
    if (mActionStatus == ACTION_STARTING) {
        ...
        m3144 = l_HIO.m1BE;
        m3146 = l_HIO.m1C0;
        setFlyingTimer(getMaxFlyingTimer());
        speedF = 0.0f;
        mAcchCir[1].SetWall(60.0f, 60.0f);
        onFlying();
        mActionStatus++;
        ...
        if (mActionStatus == ACTION_ONGOING_1) {
            ...
            calcFlyingTimer();
#if VERSION > VERSION_DEMO
            if ((dComIfGs_isEventBit(dSv_event_flag_c::UNK_3320)) && (getFlyingTimer() < 300)) {
                dComIfGs_onEventBit(dSv_event_flag_c::UNK_3310);
            }
#endif
            if (((getFlyingTimer() == 0) || (m3144 == 0)) || (CPad_CHECK_TRIG_B(0))) {
                if (checkStatus(daMdStts_UNK1)) {
                    setBitEffectStatus(1);
                }
            ...
            if ((getFlyingTimer() < l_HIO.m1C4) && (m3104 == 7)) {
                setAnm(0x2e);
                setHane03Emitter();
            }
            dAttention_c& attention = dComIfGp_getAttention();
...
void daNpc_Md_c::initialMovePosEvent(int staffIdx) {
    ...
    link = NULL;
    const char* target = dComIfGp_evmng_getMyStringP(staffIdx, "Target");
    if (target != NULL) {
        strcpy(acStack_ac, target);
        if (!strcmp(acStack_ac, "@PLAYER")) {
            link = daPy_getPlayerLinkActorClass();
        }
        if (link != NULL) {
            const char* type = dComIfGp_evmng_getMyStringP(staffIdx, "Type");
            if (type != NULL) {
                strcpy(acStack_ac, type);
            }
            if (!strcmp(acStack_ac, "LINK_DIST")) {
                uVar9 = fopAcM_searchActorAngleY(this, link);
                f32* pDist = dComIfGp_evmng_getMyFloatP(staffIdx, "Dist");
                if (pDist != NULL) {
                    f32 sin = *pDist * cM_ssin(uVar9);
                    f32 cos = *pDist * cM_scos(uVar9);
                    cXyz pos(current.pos.x + sin, current.pos.y + 50.0f, current.pos.z + cos);
                    ...
                    link->setPlayerPosAndAngle(&pos, link->shape_angle.y);
                }
                f32* pAngle = dComIfGp_evmng_getMyFloatP(staffIdx, "Angle");
                if (pAngle != NULL) {
                    link->setPlayerPosAndAngle(0, cM_deg2s(*pAngle) + (-0x8000 + uVar9));
                }
            } else {
                if (!strcmp(acStack_ac, "DIST")) {
                    f32* pDist = dComIfGp_evmng_getMyFloatP(staffIdx, "Dist");
                    if (pDist != NULL) {
                        f32 sin = *pDist * cM_ssin(link->shape_angle.y);
                        f32 cos = *pDist * cM_scos(link->shape_angle.y);
                        current.pos.set(link->current.pos.x + sin, link->current.pos.y, link->current.pos.z + cos);
                    }
                    f32* pAngle = dComIfGp_evmng_getMyFloatP(staffIdx, "Angle");
                    if (pAngle != NULL) {
                        s16 angle = cM_deg2s(*pAngle) + (-0x8000 + link->shape_angle.y);
                        current.angle.y = angle;
                        shape_angle.y = angle;
                    }
                } else {
                    f32* pAngle = dComIfGp_evmng_getMyFloatP(staffIdx, "Angle");
                    if (pAngle != NULL) {
                        s16 angle = cM_deg2s(*pAngle) + link->shape_angle.y;
                        current.angle.y = angle;
                        shape_angle.y = angle;
                    }
    ...
    speedF = 0.0f;
    speed.y = 0.0f;
    const char* pName = dComIfGp_evmng_getMyStringP(staffIdx, "Name");
    if (pName != NULL) {
        strcpy(acStack_ac, pName);
        for (s32 i = 0; i < 1; i++) {
            if (!strcmp(acStack_ac, cut_anm_tbl[i])) {
                setAnm(cut_anm_idx_tbl[i]);
                break;
            }
        }
    }
    ...
    if (pShipRide != NULL) {
        strcpy(acStack_ac, pShipRide);
        if (!strcmp(acStack_ac, "on")) {
            onShipRide();
        } else if (!strcmp(acStack_ac, "off")) {
            offShipRide();
        }
    }
}
...
void daNpc_Md_c::initialOnetimeEvent(int staffIdx) {
    ...
    const char* pName = dComIfGp_evmng_getMyStringP(staffIdx, "Name");
    if (pName != NULL) {
        strcpy(acStack_18, pName);
        for (s32 i = 0; i < 1; i++) {
            if (!strcmp(acStack_18, cut_anm_tbl[i])) {
                var_r31 = cut_anm_idx_tbl[i];
                break;
            }
        }
    }
...
void daNpc_Md_c::initialSetAnmEvent(int staffIdx) {
    ...
    if (puVar1 != NULL) {
        u8 r4 = *puVar1;
        int iVar1 = setAnm(getAnmType(r4));
        if (iVar1 != 0) {
            setBitStatus(daMdStts_UNK1);
        } else {
            clearStatus(daMdStts_UNK1);
...
BOOL daNpc_Md_c::initTexPatternAnm(u8 btpAnmTblIdx, bool param_2) {
    ...
    JUT_ASSERT(VERSION_SELECT(7355, 7487, 7502, 7502), eyeTexPtrn != NULL);
...
BOOL daNpc_Md_c::initLightBtkAnm(bool param_1) {
    ...
    JUT_ASSERT(VERSION_SELECT(7412, 7544, 7559, 7559), a_lightBtk != NULL);
    if (mLightBtkAnm.init(pModelData, a_lightBtk, true, J3DFrameCtrl::EMode_LOOP, 1.0f, 0, -1, param_1, 0)) {
        ret = true;
    }
    return ret;
}
...
BOOL daNpc_Md_c::setAnm(int anmIdx) {
    ...
    if (prm->anmTblIdx != m312D || mpMorf->getPlaySpeed() != prm->speed) {
        ...
        if (prm->armAnmTblIdx < 0x80) {
            ...
            if (!isTypeShipRide()) {
                char* tmp = wingAnmTbl[prm->armAnmTblIdx - 0x80].m00;
                dNpc_Md_setAnm(mpWingMorf, prm->loopMode, morf, speed, tmp, mModelArcName);
            }
            ret = TRUE;
        }
...
#if VERSION == VERSION_DEMO
    if (m312D == 0x0E || m312D == 0x0F || m312D == 0x12 || m312D == 0x13 || m312D == 0x1A || m312D == 0x1B || m312D == 0x1C || m312D == 0x25) {
        setBitStatus(daMdStts_UNK80);
    } else {
    ...
    if (m312D == 0x10) {
        if (m0508[1] == NULL) {
            m0508[1] = dComIfGp_particle_set(
                dPa_name::ID_IT_SN_MD_PIYOPIYO00,
                &current.pos,
                NULL,
                NULL,
                0xFF,
...
bool daNpc_Md_c::chkAttention(cXyz pos, s16 angle, int param_3) {
    ...
    cXyz delta;
    delta.x = link->current.pos.x - pos.x;
    delta.z = link->current.pos.z - pos.z;
    f32 distXZ = std::sqrtf(SQUARE(delta.x) + SQUARE(delta.z));
    s16 targetAngleY = cM_atan2s(delta.x, delta.z);
    delta.y = link->current.pos.y - pos.y;
    if (m312C != 0) {
        maxAttnDistXZ += 40.0f;
        maxAttnAngleY += 0x71c;
    }
    targetAngleY -= angle;
    if (param_3 != 0) {
        return (maxAttnDistXZ > distXZ) && (delta.y < dVar9) && (delta.y > dVar8);
    } else {
        return (maxAttnAngleY > abs(targetAngleY)) && (maxAttnDistXZ > distXZ) && (delta.y < dVar9) && (delta.y > dVar8);
    }
}
    ...
    if (isShipRide()) {
        ...
        if (checkStatus(daMdStts_UNK40)) {
            if (m312D == 0x10) {
                ...
                u8 count = getPiyo2TalkCNT();
                msgNo = l_msg_num[count];
                countPiyo2TalkCNT();
            } else {
                msgNo = 0x17F6;
            }
...
void daNpc_Md_c::setAttention(bool param_1) {
    ...
    eyePos.set(getEyePos().x, getEyePos().y, getEyePos().z);
    f32 fVar1;
    if (isFlying()) {
        fVar1 = 70.0f;
...
void daNpc_Md_c::lookBack(cXyz* param_1, int param_2, int param_3) {
    ...
    s16 desiredYRot = shape_angle.y;
    if (mJntCtrl.trnChk()) {
        s16 r4 = l_HIO.mNpc.mMaxHeadTurnVel;
        cLib_addCalcAngleS2(&m3110, r4, 4, 0x800);
    } else {
        m3110 = 0;
    }
    if (param_2 != 0) {
...
void daNpc_Md_c::setBaseMtx() {
    ...
    if (fopAcM_checkCarryNow(this)) {
        ...
        daShip_c* ship = dComIfGp_getShipActor();
        cMtx_copy(ship->getBodyMtx(), mDoMtx_stack_c::now);
    ...
    mpArmMorf->calc();
    if (checkStatus(daMdStts_UNK80)) {
        J3DModel* model = mpArmMorf->getModel();
        cMtx_copy(model->getAnmMtx(m_handL_jnt_num), mDoMtx_stack_c::now);
        mDoMtx_stack_c::transM(0.37f, 3.81f, -11.13f);
        mDoMtx_stack_c::XYZrotM(0x1b24, -0x2913, -0x1982);
    } else {
        J3DModel* model = getModel();
        cMtx_copy(model->getAnmMtx(m_backbone2_jnt_num), mDoMtx_stack_c::now);
...
void daNpc_Md_c::deletePiyoPiyo() {
    JPABaseEmitter* emitter = m0508[1];
    if (emitter == NULL) {
        return;
    }
...
BOOL daNpc_Md_c::init() {
    ...
    mCps.SetStts(&mStts);
    mCps.SetAtType(AT_TYPE_LIGHT);
    mCps.SetR(20.0f);
    ResTIMG* pBti = (ResTIMG*)dComIfG_getObjectRes(mModelArcName, "md_spot.bti");
    m0B70.init(pBti);
    if (isTypeEdaichi()) {
        onNpcCallCommand();
    }
...
BOOL daNpc_Md_c::draw() {
    ...
    if (!isTypeShipRide()) {
        if (checkStatus(daMdStts_UNK1)) {
            limbModel = mpWingMorf->getModel();
            g_env_light.setLightTevColorType(limbModel, &tevStr);
            mpWingMorf->entryDL();
        } else {
            limbModel = mpArmMorf->getModel();
            g_env_light.setLightTevColorType(limbModel, &tevStr);
            mpArmMorf->entryDL();
        }
    } else {
...
#endif
        dComIfGd_getXluList()->entryImm(&m0B70, DEMO_SELECT(0, 0x1F));
    }
...
#endif
    {
        ...
        mShadowId = dComIfGd_setShadow(mShadowId, 0, getModel(), &shadowPos, 800.0f, 20.0f, current.pos.y, mAcch.GetGroundH(), mAcch.m_gnd, &tevStr);

        if (mShadowId != 0) {
            dComIfGd_addRealShadow(mShadowId, limbModel);
...
void daNpc_Md_c::animationPlay() {
    ...
    if (((isTypeSea()) && (bVar3)) && (m3104 == 0x16 || (m3104 == 0x17))) {
        BOOL bVar2 = false;
        if (m3104 == 0x16) {
    ...
    if (mpMorf->getPlaySpeed() < 0.0f) {
        if (m30D0 < mpMorf->getFrame()) {
            m312A = 1;
        }
    } else if (m30D0 > mpMorf->getFrame()) {
        m312A = 1;
    }
    m30D0 = mpMorf->getFrame();
#if VERSION > VERSION_DEMO
...
BOOL daNpc_Md_c::execute() {
    ...
    if (isTypeM_DaiB()) {
        ...
        if ((mAcch.ChkWaterIn()) && (mAcch.m_wtr.GetHeight() > current.pos.y + 5.0f)) {
            if (m3074.getEmitter() == NULL) {
                dComIfGp_particle_setShipTail(dPa_name::ID_AK_JN_HAMON00, &current.pos, NULL, NULL, 0xFF, &m3074);
            }
            m3074.setRate(0.0f);
            bVar10 = true;
        }
    ...
    setBaseMtx();
    model = getModel();
    emitterTrace(m0508[0], model->getAnmMtx(m_neck_jnt_num), &current.angle);
    emitterTrace(m0508[1], model->getAnmMtx(m_neck_jnt_num), NULL);
    if (!isTypeShipRide()) {
        model = mpWingMorf->getModel();
        emitterTrace(m0508[2], model->getAnmMtx(m_wingR3_jnt_num), NULL);
        emitterTrace(m0508[3], model->getAnmMtx(m_wingL3_jnt_num), NULL);
        emitterTrace(m0508[4], model->getAnmMtx(m_wingR3_jnt_num), NULL);
        emitterTrace(m0508[5], model->getAnmMtx(m_wingL3_jnt_num), NULL);
        if (checkBitEffectStatus(1)) {
            model = mpWingMorf->getModel();
            local_3c.set(model->getAnmMtx(m_wingR2_jnt_num)[0][3], model->getAnmMtx(m_wingR2_jnt_num)[1][3], model->getAnmMtx(m_wingR2_jnt_num)[2][3]);
            dComIfGp_particle_set(
                dPa_name::ID_IT_SN_MD_HANE01,
                &local_3c,
                NULL,
                NULL,
                0xFF,
            ...
            local_3c.set(model->getAnmMtx(m_wingL2_jnt_num)[0][3], model->getAnmMtx(m_wingL2_jnt_num)[1][3], model->getAnmMtx(m_wingL2_jnt_num)[2][3]);
            dComIfGp_particle_set(
                dPa_name::ID_IT_SN_MD_HANE01,
                &local_3c,
                NULL,
