...
BOOL daPy_lk_c::checkMabaAnimeBtp(int anmIdx) {
    if (anmIdx >= dRes_INDEX_LKANM_BTP_TMABAA_e && anmIdx <= dRes_INDEX_LKANM_BTP_TMABAJ_e) {
        return TRUE;
    }
    return FALSE;
}
...
u16 daPy_lk_c::checkNormalFace() {
    ...
    if (checkUpperAnime(dRes_INDEX_LKANM_BCK_GRABWAITB_e)) {
        return daPyFace_TMABAJ_TEYORIME;
    }
    if (checkUpperAnime(dRes_INDEX_LKANM_BCK_GRABWAIT_e)) {
    ...
    if (m_anm_heap_under[UNDER_MOVE1_e].mIdx == dRes_INDEX_LKANM_BCK_WALKSLOPE_e) {
        return daPyFace_TMABAC;
    }
    if (mCurProc == daPyProc_MOVE_e) {
        ...
        if ((mNormalSpeed * fVar2) / mMaxNormalSpeed >= 0.9f) {
            return daPyFace_TMABACC;
        }
    }
    if (mCurProc == daPyProc_WAIT_e && !dComIfGp_event_runCheck()) {
        f32 dVar7 = cM_rnd();
        if (((m_tex_anm_heap.mIdx == dRes_INDEX_LKANM_BTP_TMABAA_e || m_tex_anm_heap.mIdx == dRes_INDEX_LKANM_BTP_TMABAB_e) && dVar7 < 0.01f) ||
            (m_tex_anm_heap.mIdx == dRes_INDEX_LKANM_BTP_TMABACC_e && dVar7 >= 0.01f))
        {
            return daPyFace_TMABACC;
        }
...
void daPy_lk_c::setTextureAnime(u16 param_1, int param_2) {
    ...
    if (m_tex_anm_heap.mIdx == btp_idx) {
        if (checkFaceTypeNot() && !checkModeFlg(ModeFlg_00000100 | ModeFlg_00000400)) {
            m3530 = param_2;
        }
    } else {
    ...
    if (m_tex_scroll_heap.mIdx != btk_idx) {
        if (bVar3 && checkModeFlg(ModeFlg_00000001) &&
            daPy_matAnm_c::getMabaFlg() != 0 &&
            !dComIfGp_event_runCheck() &&
            mpAttnActorLockOn == NULL &&
            btk_idx == dRes_INDEX_LKANM_BTK_TMABA_e)
        {
            if (daPy_matAnm_c::getMabaTimer() == 0) {
                if (dVar9 < 0.44f) {
                    ...
                    param_2 = 0;
                    if (dVar9 < 0.51f) {
                        btk_idx = dRes_INDEX_LKANM_BTK_TEUP_e;
                    } else if (dVar9 < 0.58f) {
                        btk_idx = dRes_INDEX_LKANM_BTK_TEDW_e;
                    } else if (dVar9 < 0.65f) {
                        btk_idx = dRes_INDEX_LKANM_BTK_TEL_e;
                    } else if (dVar9 < 0.72f) {
                        btk_idx = dRes_INDEX_LKANM_BTK_TER_e;
                    } else if (dVar9 < 0.79f) {
                        btk_idx = dRes_INDEX_LKANM_BTK_TEDL_e;
                    } else if (dVar9 < 0.86f) {
                        btk_idx = dRes_INDEX_LKANM_BTK_TEDR_e;
                    } else if (dVar9 < 0.93f) {
                        btk_idx = dRes_INDEX_LKANM_BTK_TEUR_e;
                    } else {
                        btk_idx = dRes_INDEX_LKANM_BTK_TEUL_e;
                    }
                }
                ...
                if (btk_idx != dRes_INDEX_LKANM_BTK_TMABA_e) {
            ...
            if (m_tex_scroll_heap.field_0x2 == 0xFFFF && m_tex_scroll_heap.field_0x4 == 0xFFFF)
            {
               ...
               daPy_matAnm_c::getMabaTimer() == 0 && mpAttnActorLockOn == NULL && btk_idx == dRes_INDEX_LKANM_BTK_TMABA_e &&
               m_tex_scroll_heap.field_0x2 == 0xFFFF && m_tex_scroll_heap.field_0x4 == 0xFFFF && cM_rnd() < 0.025f)
    {
        if (dVar9 < 0.125f) {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TEUP_e;
        } else if (dVar9 < 0.25f) {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TEDW_e;
        } else if (dVar9 < 0.375f) {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TEL_e;
        } else if (dVar9 < 0.5f) {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TER_e;
        } else if (dVar9 < 0.625f) {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TEDL_e;
        } else if (dVar9 < 0.75f) {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TEDR_e;
        } else if (dVar9 < 0.875f) {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TEUL_e;
        } else {
            m_tex_scroll_heap.mIdx = dRes_INDEX_LKANM_BTK_TEUR_e;
        }
...
void daPy_lk_c::playTextureAnime() {
    if (m_tex_anm_heap.field_0x4 != 0xFFFF) {
        ...
        if (m_tex_scroll_heap.mIdx == dRes_INDEX_LKANM_BTK_TABEKOBE_e) {
            m3532++;
...
void daPy_lk_c::setBootsModel(J3DModel** pBootModels) {
    ...
    if (checkEquipHeavyBoots()) {
        pBootModels[0]->setAnmMtx(HBOOTS_JNT_CL_HBOOTSA_e, mpCLModel->getAnmMtx(CL_JNT_RLEGB_JNT_e));
        pBootModels[1]->setAnmMtx(HBOOTS_JNT_CL_HBOOTSA_e, mpCLModel->getAnmMtx(CL_JNT_LLEGB_JNT_e));
        bootsJointIdx = HBOOTS_JNT_CL_HBOOTSB_e;
    } else {
        bootsJointIdx = HBOOTS_JNT_CL_HBOOTSA_e;
    }
    
    J3DModel* bootModel = pBootModels[0];
    bootModel->setAnmMtx(bootsJointIdx, mpCLModel->getAnmMtx(CL_JNT_RTOE_JNT_e));
    bootModel->setAnmMtx(bootsJointIdx+1, mpCLModel->getAnmMtx(CL_JNT_RFOOT_JNT_e));
    
    bootModel = pBootModels[1];
    bootModel->setAnmMtx(bootsJointIdx, mpCLModel->getAnmMtx(CL_JNT_LTOE_JNT_e));
    bootModel->setAnmMtx(bootsJointIdx+1, mpCLModel->getAnmMtx(CL_JNT_LFOOT_JNT_e));
}

/* 80105910-80106660       .text setItemModel__9daPy_lk_cFv */
s32 daPy_lk_c::setItemModel() {
    MtxP podA_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_CL_PODA_e);
    MtxP lHandA_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_CL_LHANDA_e);
    MtxP rHandA_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_CL_RHANDA_e);
    ...
    mpPodmsModel->setBaseTRMtx(podA_jnt_mtx);
    if (mpEquipItemModel != NULL) {
        if (mEquipItem == daPyItem_UNK10A_e) {
            ...
            mDoMtx_stack_c::revConcat(dComIfGp_getShipActor()->getTactJntMtx());
            ...
            mDoMtx_stack_c::revConcat(mpCLModel->getAnmMtx(CL_JNT_STOMACH_JNT_e));
            mpEquipItemModel->setBaseTRMtx(mDoMtx_stack_c::get());
        } else if (mEquipItem == dItemNo_TELESCOPE_e) {
            mDoMtx_stack_c::transS(8.0f, 15.0f, 1.0f);
            mDoMtx_stack_c::XYZrotM(-0x4000, 0x4444, -0x41F);
            mDoMtx_stack_c::revConcat(lHandA_jnt_mtx);
            mpEquipItemModel->setBaseTRMtx(mDoMtx_stack_c::get());
        } else if (mEquipItem == dItemNo_HYOI_PEAR_e) {
            if (mFrameCtrlUnder[UNDER_MOVE0_e].getFrame() < 9.0f) {
                ...
                mpEquipItemModel->setBaseTRMtx(mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e));
            }
        } else if (mEquipItem == dItemNo_DEKU_LEAF_e || mEquipItem == daPyItem_SWORD_e ||
                   mEquipItem == dItemNo_WIND_WAKER_e || mEquipItem == dItemNo_SKULL_HAMMER_e ||
                   mEquipItem == dItemNo_HOOKSHOT_e || mEquipItem == dItemNo_TINGLE_TUNER_e ||
                   checkBottleItem(mEquipItem) || checkPhotoBoxItem(mEquipItem))
        {
            mpEquipItemModel->setBaseTRMtx(lHandA_jnt_mtx);
                ...
                (mCurProc != daPyProc_FAN_SWING_e && mCurProc != daPyProc_FAN_GLIDE_e))
            {
                ...
                if (mpBottleContentsModel != NULL) {
                    if (mEquipItem == dItemNo_FIREFLY_BOTTLE_e) {
                        ...
                        mpBottleContentsModel->setBaseScale(scale);
                        simpleAnmPlay(mpEquipItemBrk);
                    } else {
                        mpBottleContentsModel->setBaseTRMtx(lHandA_jnt_mtx);
                    }
                }
                if (mpBottleCapModel != NULL) {
                    mpBottleCapModel->setBaseTRMtx(lHandA_jnt_mtx);
                }
            } else if (mEquipItem == dItemNo_HOOKSHOT_e && boko != NULL) {
                if (fopAcM_GetParam(boko) != daBoko_c::Type_BOKO_STICK_e) {
                    ...
                    f32 fVar2 = mSwordAnim.getBckAnm()->getFrameMax();
                    if (m35EC < 0.0f) {
                        m35EC += fVar2;
                    } else if (m35EC >= fVar2) {
                        m35EC -= fVar2;
            ...
            } else if (mEquipItem == dItemNo_WIND_WAKER_e &&
                       mCurProc != daPyProc_TACT_WAIT_e && mpEquipItemBrk != NULL)
            {
                f32 fVar1 = mpEquipItemBrk->getFrame() - 1.0f;
                if (fVar1 < 0.0f) {
                    fVar1 = 0.0f;
                }
            ...
            mpEquipItemModel->setBaseTRMtx(rHandA_jnt_mtx);
        } else if (mEquipItem == daPyItem_UNK102_e) {
            mpEquipItemModel->setBaseTRMtx(mpCLModel->getAnmMtx(CL_JNT_CHEST_JNT_e));
        }
        ...
        if (checkNormalSwordEquip()) {
            ...
            if (mpEquipItemModel != NULL && mEquipItem == daPyItem_SWORD_e) {
                mpEquipItemModel->getModelData()
                    ->getJointNodePointer(SWA_JNT_CL_SWA_e)
                    ->getMesh()
                    ->getShape()
                    ->show();
            }
        } else {
            if (dComIfGs_getSelectEquip(0) == dItemNo_MASTER_SWORD_2_e) {
                mSwgripmsabBckAnim.entry(mpSwgripmsModel->getModelData(), 1.0f);
                r27 = 4;
            } else {
                mSwgripmsabBckAnim.entry(mpSwgripmsModel->getModelData(), 0.0f);
    ...
    f32 dVar16 = mpTswgripmsBtk->getFrame();
    ...
    if (mCurProc == daPyProc_DEMO_LAST_COMBO_e) {
        ...
        mDoAud_setLinkShieldType(r27_2, 1);
               ...
               (mCurProc == daPyProc_DEMO_TOOL_e && mProcVar2.m34D4 == 1) ||
               (checkPlayerGuard() && mEquipItem != dItemNo_SKULL_HAMMER_e) ||
               mCurProc == daPyProc_GUARD_CRASH_e)
    {
        mpEquippedShieldModel->setBaseTRMtx(rHandA_jnt_mtx);
        mDoAud_setLinkShieldType(r27_2, 1);
    } else {
        if (r27_2 == 2) {
        ...
        if (r27_2 == 2 && (m_anm_heap_under[UNDER_MOVE0_e].mIdx == dRes_INDEX_LKANM_BCK_WALK_e ||
                           m_anm_heap_under[UNDER_MOVE0_e].mIdx == dRes_INDEX_LKANM_BCK_WALKS_e ||
                           m_anm_heap_under[UNDER_MOVE0_e].mIdx == dRes_INDEX_LKANM_BCK_DASHS_e ||
                           m_anm_heap_under[UNDER_MOVE0_e].mIdx == dRes_INDEX_LKANM_BCK_DASH_e))
        {
            Vec temp;
            ...
            mDoMtx_stack_c::YrotS(
                (-5500.0f * std::abs(mNormalSpeed / mMaxNormalSpeed) * cM_ssin(uVar10 & 0xFFFF)));
    ...
    if (checkPowerGloveEquip()) {
        mpPringModel->setAnmMtx(PRING_JNT_PRINGL_e, mpCLModel->getAnmMtx(CL_JNT_LARMB_JNT_e));
        mpPringModel->setAnmMtx(PRING_JNT_PRINGR_e, mpCLModel->getAnmMtx(CL_JNT_RARMB_JNT_e));
    }
    return false;
}
...
s32 daPy_lk_c::setDrawHandModel() {
    ...
    mpRhandShape->hide();
    if (mCurProc == daPyProc_DEMO_TOOL_e) {
        lHand_JntNo = mLeftHandIdx;
        rHand_JntNo = mRightHandIdx;
    } else if (checkGrabAnime() || checkBoomerangThrowAnime() || checkBoomerangCatchAnime()) {
        lHand_JntNo = HANDS_JNT_WORLD_ROOT_e;
        rHand_JntNo = HANDS_JNT_WORLD_ROOT_e;
    } else if (checkHookshotReadyAnime()) {
        lHand_JntNo = HANDS_JNT_CL_LHANDD_e;
        rHand_JntNo = HANDS_JNT_WORLD_ROOT_e;
    } else if (checkBowReadyAnime()) {
        lHand_JntNo = HANDS_JNT_CL_LHANDD_e;
        rHand_JntNo = HANDS_JNT_CL_RHANDE_e;
    } else if (checkBowShootAnime()) {
        lHand_JntNo = HANDS_JNT_CL_LHANDG_e;
        rHand_JntNo = HANDS_JNT_CL_RHANDE_e;
    } else if (checkBoomerangReadyAnime()) {
        lHand_JntNo = HANDS_JNT_WORLD_ROOT_e;
        rHand_JntNo = HANDS_JNT_CL_RHANDB_e;
    } else if (checkBottleItem(mEquipItem) || checkPhotoBoxItem(mEquipItem)) {
        lHand_JntNo = HANDS_JNT_CL_LHANDG_e;
        rHand_JntNo = mRightHandIdx;
    } else if (checkBowItem(mEquipItem)) {
        lHand_JntNo = mLeftHandIdx;
        rHand_JntNo = HANDS_JNT_CL_RHANDC_e;
    } else if (mEquipItem == daPyItem_BOKO_e) {
        lHand_JntNo = HANDS_JNT_CL_LHANDD_e;
        rHand_JntNo = HANDS_JNT_CL_RHANDD_e;
    ...
    } else if ((mEquipItem == dItemNo_GRAPPLING_HOOK_e && !checkModeFlg(ModeFlg_ROPE)) ||
               (mEquipItem == dItemNo_SKULL_HAMMER_e && mCutType == CUT_TYPE_NONE))
    {
        lHand_JntNo = HANDS_JNT_CL_LHANDD_e;
        rHand_JntNo = HANDS_JNT_CL_RHANDC_e;
        ...
        lHand_JntNo = HANDS_JNT_CL_LHANDF_e;
        rHand_JntNo = mRightHandIdx;
    } else if (mEquipItem == dItemNo_HOOKSHOT_e) {
        lHand_JntNo = HANDS_JNT_CL_LHANDC_e;
        rHand_JntNo = mRightHandIdx;
    } else if (mEquipItem == dItemNo_DEKU_LEAF_e) {
        lHand_JntNo = HANDS_JNT_CL_LHANDD_e;
        rHand_JntNo = mRightHandIdx;
    } else if (mEquipItem == dItemNo_TELESCOPE_e) {
        lHand_JntNo = HANDS_JNT_CL_LHANDB_e;
        rHand_JntNo = mRightHandIdx;
    } else {
        if (mEquipItem == daPyItem_NONE_e && checkUpperAnime(dRes_INDEX_LKANM_BCK_TAKEBOTH_e)) {
            lHand_JntNo = HANDS_JNT_CL_LHANDB_e;
        ...
        if (checkPlayerGuard() || mCurProc == daPyProc_GUARD_CRASH_e) {
            rHand_JntNo = HANDS_JNT_CL_RHANDC_e;
        } else if (mEquipItem == daPyItem_NONE_e && checkUpperAnime(dRes_INDEX_LKANM_BCK_TAKEBOTH_e)) {
            rHand_JntNo = HANDS_JNT_CL_RHANDB_e;
        } else if (mEquipItem == daPyItem_NONE_e && checkItemEquipAnime()) {
            rHand_JntNo = HANDS_JNT_WORLD_ROOT_e;
        } else {
    ...
    if (lHand_JntNo == HANDS_JNT_WORLD_ROOT_e) {
        mpLhandShape = mpCLModelData->getJointNodePointer(CL_JNT_CL_LHANDA_e)->getMesh()->getShape();
    } else {
        mpLhandShape =
            mpHandsModel->getModelData()->getJointNodePointer(lHand_JntNo)->getMesh()->getShape();
        mpHandsModel->setAnmMtx(lHand_JntNo, mpCLModel->getAnmMtx(CL_JNT_CL_LHANDA_e));
    }
    mpLhandShape->show();
    if (rHand_JntNo == HANDS_JNT_WORLD_ROOT_e) {
        mpRhandShape = mpCLModelData->getJointNodePointer(CL_JNT_CL_RHANDA_e)->getMesh()->getShape();
    } else {
        mpRhandShape =
            mpHandsModel->getModelData()->getJointNodePointer(rHand_JntNo)->getMesh()->getShape();
        mpHandsModel->setAnmMtx(rHand_JntNo, mpCLModel->getAnmMtx(CL_JNT_CL_RHANDA_e));
    }
    mpRhandShape->show();
    return true;
}
...
void daPy_lk_c::drawShadow() {
    J3DJoint* link_root_joint = mpCLModelData->getJointNodePointer(CL_JNT_LINK_ROOT_e);
    MtxP root_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_LINK_ROOT_e);
    cXyz root_jnt_pos;

    root_jnt_pos.x = root_jnt_mtx[0][3];
    root_jnt_pos.y = root_jnt_mtx[1][3];
    root_jnt_pos.z = root_jnt_mtx[2][3];
    if (checkGrabWear()) {
        if (-G_CM3D_F_INF != mAcch.GetGroundH() && dComIfG_Bgsp()->ChkPolySafe(mAcch.m_gnd)) {
            dComIfGd_setSimpleShadow2(&root_jnt_pos, mAcch.GetGroundH(), 30.0f, mAcch.m_gnd, 0, 1.0f,
                                      &dDlst_shadowControl_c::mSimpleTexObj);
        }
    } else {
        mpCLModelData->getJointNodePointer(CL_JNT_CL_LHANDA_e)->getMesh()->getShape()->show();
        mpCLModelData->getJointNodePointer(CL_JNT_CL_RHANDA_e)->getMesh()->getShape()->show();
        if (checkNoResetFlg1(daPyFlg1_CASUAL_CLOTHES)) {
        ...
        mShadowId = dComIfGd_setShadow(mShadowId, 0, mpCLModel, &root_jnt_pos, f1, 30.0f, current.pos.y,
        ...
        if (mShadowId != 0) {
            ...
            if (mActorKeepGrab.getID() != fpcM_ERROR_PROCESS_ID_e) {
                fopAc_ac_c* grab_actor = fopAcM_SearchByID(mActorKeepGrab.getID());
                if (grab_actor != NULL && fopAcM_GetModel(grab_actor) != NULL) {
                    dComIfGd_addRealShadow(mShadowId, fopAcM_GetModel(grab_actor));
                }
            } else if (mActorKeepEquip.getID() != fpcM_ERROR_PROCESS_ID_e) {
                fopAc_ac_c* equip_actor = fopAcM_SearchByID(mActorKeepEquip.getID());
                if (equip_actor != NULL && fopAcM_GetModel(equip_actor) != NULL) {
                    dComIfGd_addRealShadow(mShadowId, fopAcM_GetModel(equip_actor));
                }
            }
        }
    }
...
BOOL daPy_lk_c::draw() {
    ...
    J3DJoint* link_root_joint = mpCLModelData->getJointNodePointer(CL_JNT_LINK_ROOT_e);
    mpCLModelData->getJointNodePointer(CL_JNT_CL_LHANDA_e)->getMesh()->getShape()->hide();
    mpCLModelData->getJointNodePointer(CL_JNT_CL_RHANDA_e)->getMesh()->getShape()->hide();
    ...
    J3DJoint* cl_eye_joint = mpCLModelData->getJointNodePointer(CL_JNT_CL_EYE_e);
    J3DJoint* cl_mayu_joint = mpCLModelData->getJointNodePointer(CL_JNT_CL_MAYU_e);
    setDrawHandModel();
    ...
    if (r24) {
        ...
        mpCLModelData->getJointNodePointer(CL_JNT_CL_HANA_e)->getMesh()->getShape()->hide();
    } else {
        if (!checkFreezeState()) {
            dComIfGd_setListP0();
        ...
        if (
            ...
            mpCLModelData->getJointNodePointer(CL_JNT_CL_PODA_e)->getMesh()->getShape()->hide();
        } else {
            mpCLModelData->getJointNodePointer(CL_JNT_CL_PODA_e)->getMesh()->getShape()->show();
        }
    }
    ...
    mpCLModelData->getJointNodePointer(CL_JNT_CL_HANA_e)->getMesh()->getShape()->show();
    mpCLModelData->getJointNodePointer(CL_JNT_CL_BACK_e)->getMesh()->getShape()->show();
    if (!r24) {
    ...
    if (!r24) {
        ...
        } else if (checkModeFlg(ModeFlg_SWIM) && checkNoResetFlg0(daPyFlg0_UNK100) && !(mCurProc == daPyProc_DEMO_DEAD_e && mProcVar3.m34D6 == 0)) {
            GXColor spc;
            GXColor sp8;
        ...
        if (checkShieldEquip() && !checkCaughtShapeHide()
#if VERSION == VERSION_DEMO
            && !checkLastDemoSwordNoDraw(1)
#else
            && !checkDemoShieldNoDraw()
...
int daPy_lk_c::setHintActor() {
    if (dComIfGp_att_getZHint() != 0 && !dComIfGp_event_runCheck() && !checkPlayerDemoMode()) {
        setDoButtonQuake();
...
s16 daPy_lk_c::checkBodyAngleX(s16 param_1) {
    ...
    mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_BODY_CHN_e), &sp38);
...
void daPy_lk_c::setBodyAngleXReadyAnime() {
    ...
    mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_BODY_CHN_e), &local_1c);
    if (mpAttnActorLockOn != NULL) {
    ...
    if (
        (
            ...
            (
                ...
                fopAcM_GetName(mpAttnActorLockOn) == fpcNm_BOOMERANG_e
            )
        ) ||
...
void daPy_lk_c::setBlendMoveAnime(f32 param_1) {
        ...
        ((f1_2 >= 25.0f || ((f1_2 >= 0.09f && ((m34C3 == 9 || m34C3 == 10)))))))
    {
        ...
        if (f30 < m_HIO->mMove.m.field_0x2C || checkHeavyStateOn()) {
            f32 f25_2;
            if (checkHeavyStateOn()) {
                f25_2 = f30 / m_HIO->mMove.m.field_0x80;
                if (f25_2 > 0.55f) {
                    r25 = true;
                }
            } else {
            ...
            if (checkModeFlg(ModeFlg_00000001)) {
                if (checkAttentionLock()) {
                    if (checkUpperGuardAnime()) {
                        setMoveAnime(0.0f, m_HIO->mAtnMove.m.field_0x24, m_HIO->mAtnMove.m.field_0x28, ANM_ATNRS, ANM_ATNWRS, 2, m_HIO->mBasic.m.field_0xC);
                        return;
                    }
                }
                f32 f1 = m3598 = 0.0f;
                    ...
                    !dComIfGp_event_runCheck())
                {
                    if (m_anm_heap_under[UNDER_MOVE0_e].mIdx != dRes_INDEX_LKANM_BCK_WAITQ_e) {
                        setSingleMoveAnime(ANM_WAITQ, m_HIO->mMove.m.field_0x84, f1, -1, m_HIO->mBasic.m.field_0xC);
                        m34C3 = 2;
                    }
                    return;
                }
...
void daPy_lk_c::setAnimeEquipItem() {
    if (mActorKeepThrow.getActor() != NULL) {
        if (getReadyItem() == dItemNo_BOOMERANG_e) {
            return;
        }
        daBoomerang_c* boomerang = (daBoomerang_c*)mActorKeepThrow.getActor();
        boomerang->onCancelFlg();
...
void daPy_lk_c::setAnimeUnequipSword() {
    ...
    setActAnimeUpper(dRes_INDEX_LKANM_BCK_REST_e, UPPER_MOVE2_e, rate, start, end, f3);
    setPriTextureAnime(0x72, 0);
}

/* 8010C284-8010C3C0       .text setAnimeUnequipItem__9daPy_lk_cFUs */
void daPy_lk_c::setAnimeUnequipItem(u16 i_itemNo) {
    if (i_itemNo == dItemNo_BOOMERANG_e ||
        i_itemNo == dItemNo_GRAPPLING_HOOK_e ||
        i_itemNo == dItemNo_TELESCOPE_e ||
        checkPhotoBoxItem(i_itemNo) ||
        i_itemNo == dItemNo_HOOKSHOT_e ||
        i_itemNo == dItemNo_DEKU_LEAF_e ||
        i_itemNo == dItemNo_TINGLE_TUNER_e ||
        i_itemNo == dItemNo_WIND_WAKER_e ||
        checkBottleItem(i_itemNo)
    ) {
        setAnimeEquipSingleItem(dRes_INDEX_LKANM_BCK_TAKEL_e);
    } else if (checkBowItem(i_itemNo)) {
        setAnimeEquipSingleItem(dRes_INDEX_LKANM_BCK_TAKER_e);
    } else if (i_itemNo == dItemNo_SKULL_HAMMER_e) {
        setActAnimeUpper(
            dRes_INDEX_LKANM_BCK_TAKEBOTH_e,
            UPPER_MOVE2_e,
            m_HIO->mItem.m.field_0x24,
            m_HIO->mItem.m.field_0x28,
            m_HIO->mItem.m.field_0x4,
            m_HIO->mItem.m.field_0x2C
        );
    } else {
        setActAnimeUpper(
            dRes_INDEX_LKANM_BCK_TAKE_e,
            UPPER_MOVE2_e,
            m_HIO->mItem.m.field_0x8,
            m_HIO->mItem.m.field_0xC,
            m_HIO->mItem.m.field_0x0,
            m_HIO->mItem.m.field_0x10
        );
...
void daPy_lk_c::deleteEquipItem(BOOL param_1) {
    ...
    if ((mEquipItem == dItemNo_GRAPPLING_HOOK_e && checkRopeAnime()) ||
        (mEquipItem == dItemNo_BOOMERANG_e && checkBoomerangAnime()) ||
        (mEquipItem == dItemNo_HOOKSHOT_e && checkHookshotReadyAnime()) ||
...
void daPy_lk_c::setFallVoice() {
    if (!checkNoResetFlg0(daPyFlg0_NO_FALL_VOICE) &&
        (!checkNoResetFlg0(daPyFlg0_UNK80) || mWaterY < mAcch.GetGroundH() || m3580 == 4 ||
...
void daPy_lk_c::makeItemType() {
    if (mEquipItem == dItemNo_GRAPPLING_HOOK_e) {
        fopAc_ac_c* rope = (fopAc_ac_c*)fopAcM_fastCreate(fpcNm_HIMO2_e, 0, &current.pos);
        mActorKeepEquip.setData(rope);
    } else if (mEquipItem == dItemNo_HOOKSHOT_e) {
        fopAc_ac_c* hookshot = (fopAc_ac_c*)fopAcM_fastCreate(fpcNm_HOOKSHOT_e, 0, &current.pos);
        mActorKeepEquip.setData(hookshot);
        setHookshotModel();
    } else if (mEquipItem == dItemNo_BOOMERANG_e) {
        fopAc_ac_c* boomerang = (fopAc_ac_c*)fopAcM_fastCreate(fpcNm_BOOMERANG_e, 0, &current.pos);
        mActorKeepEquip.setData(boomerang);
    } else if (mEquipItem == dItemNo_BOMB_BAG_e) {
        fopAc_ac_c* bomb = (fopAc_ac_c*)fopAcM_fastCreate(fpcNm_BOMB_e, daBomb_c::prm_make(daBomb_c::STATE_3, false, false), &current.pos);
        mActorKeepGrab.setData(bomb);
        mEquipItem = daPyItem_NONE_e;
        if (mActorKeepGrab.getActor() != NULL) {
            setActAnimeUpper(dRes_INDEX_LKANM_BCK_GRABWAIT_e, UPPER_MOVE2_e, 0.0f, 0.0f, -1, 5.0f);
            mActivePlayerBombs++;
        ...
        setPhotoBoxModel();
        return;
    } else if (mEquipItem == dItemNo_TINGLE_TUNER_e) {
...
void daPy_lk_c::setScopeModel() {
    ...
    initModel(&mpEquipItemModel, dRes_INDEX_LINK_BDL_TELESCOPE_e, 0x37221222);
    mDoExt_setCurrentHeap(oldHeap);
}
...
void daPy_lk_c::setPhotoBoxModel() {
    JKRHeap* oldHeap = setItemHeap();
    initModel(&mpEquipItemModel, dRes_INDEX_LINK_BDL_CAMERA_e, 0x37221222);
    mDoExt_setCurrentHeap(oldHeap);
    if (mEquipItem == dItemNo_PICTO_BOX_e) {
        mpEquipItemModel->getModelData()->getJointNodePointer(CAMERA_JNT_FRASH_e)->getMesh()->getShape()->hide();
    } else {
        mpEquipItemModel->getModelData()->getJointNodePointer(CAMERA_JNT_FRASH_e)->getMesh()->getShape()->show();
    }
}
...
BOOL daPy_lk_c::checkNewItemChange(u8 i_itemBtn) {
    u16 itemNo = dComIfGp_getSelectItem(i_itemBtn);

    if (itemNo == dItemNo_MAGIC_ARMOR_e) {
        return changeDragonShield(1);
    }
    if (itemNo == dItemNo_IRON_BOOTS_e || itemNo == dItemNo_WATER_BOOTS_e || itemNo == dItemNo_TINGLE_TUNER_e || itemNo == dItemNo_SAIL_e ||
        isEsa(itemNo) || isDaizaItem(itemNo) || isEmono(itemNo) || checkBottleItem(itemNo) ||
        (itemNo >= dItemNo_FATHER_LETTER_e && itemNo <= dItemNo_FILL_UP_COUPON_e))
    {
        if (mAcch.ChkGroundHit() && (!daPy_lk_c::checkPlayerFly())) {
            if (itemNo == dItemNo_IRON_BOOTS_e || itemNo == dItemNo_WATER_BOOTS_e) {
                procBootsEquip_init(itemNo);
            } else {
                if (isEsa(itemNo)) {
    ...
    if (mEquipItem != itemNo && itemNo != dItemNo_NONE_e) {
        if (itemNo == dItemNo_BOMB_BAG_e &&
            (mActivePlayerBombs >= 3 || dComIfGs_getBombNum() == 0))
        {
            seStartSystem(JA_SE_ITEM_TARGET_OUT);
            return false;
        } else {
            mReadyItemBtn = i_itemBtn;
            setAnimeEquipItem();
        }
    }
    return false;
    ...
    if (checkModeFlg(ModeFlg_00000004) &&
        ...
        if (!daPy_dmEcallBack_c::checkCurse()) {
            if (swordTrigger() && checkSwordEquip() && mEquipItem != daPyItem_SWORD_e) {
                ...
                if (checkNewItemChange(dItemBtn_Y_e)) {
                    mReadyItemBtn = dItemBtn_Y_e;
                    return TRUE;
                }
            } else if (itemTriggerZ()) {
                if (checkNewItemChange(dItemBtn_Z_e)) {
                    mReadyItemBtn = dItemBtn_Z_e;
                    return TRUE;
                }
            } else if (doTrigger()) {
...
void daPy_lk_c::checkItemAction() {
        ...
        checkUpperAnime(dRes_INDEX_LKANM_BCK_ROPETHROW_e))
    {
        ...
        } else if (checkUpperAnime(dRes_INDEX_LKANM_BCK_ROPETHROW_e)) {
...
int daPy_lk_c::orderTalk() {
        ...
        itemTriggerX())
    {
        if (((isEsa(dComIfGp_getSelectItem(dItemBtn_X_e))) &&
             dComIfGp_getSelectItem(dItemBtn_X_e) != dItemNo_HYOI_PEAR_e) &&
            m3630 != fpcM_ERROR_PROCESS_ID_e)
        {
            seStartSystem(JA_SE_ITEM_TARGET_OUT);
            return false;
        ...
        itemTriggerY())
    {
        if (((isEsa(dComIfGp_getSelectItem(dItemBtn_Y_e))) &&
             dComIfGp_getSelectItem(dItemBtn_Y_e) != dItemNo_HYOI_PEAR_e) &&
            m3630 != fpcM_ERROR_PROCESS_ID_e)
        {
            seStartSystem(JA_SE_ITEM_TARGET_OUT);
            return false;
                                    ...
                                    itemTriggerZ())))
    {
        if (((isEsa(dComIfGp_getSelectItem(dItemBtn_Z_e))) &&
             dComIfGp_getSelectItem(dItemBtn_Z_e) != dItemNo_HYOI_PEAR_e) &&
            m3630 != fpcM_ERROR_PROCESS_ID_e)
        {
            seStartSystem(JA_SE_ITEM_TARGET_OUT);
            return false;
...
void daPy_lk_c::setShieldGuard() {
    ...
    if (checkGuardSlip() ||
        (!checkEquipAnime() && !checkGrabAnime() && checkAttentionLock() && mEquipItem != daPyItem_BOKO_e &&
         checkShieldEquip() && checkGuardAccept() &&
    ...
    u16 uVar3;
    if (mEquipItem == dItemNo_SKULL_HAMMER_e) {
        uVar3 = dRes_INDEX_LKANM_BCK_ATNGHAM_e;
    } else {
        uVar3 = dRes_INDEX_LKANM_BCK_ATNG_e;
    }
    if (!checkGuardSlip()) {
        if (spActionButton() && dComIfGp_getRStatus() == dActStts_DEFEND_e) {
            ...
            if (!checkUpperAnime(uVar3)) {
                ...
                if (local_18 < 1.0f) {
                    if (m_HIO->mBasic.m.field_0xC > 0.01f) {
                        cLib_chaseF(&local_18, 1.0f, 1.0f / m_HIO->mBasic.m.field_0xC);
                    } else {
                        local_18 = 1.0f;
                    }
            ...
            if (m_anm_heap_under[UNDER_MOVE0_e].mIdx == dRes_INDEX_LKANM_BCK_ATNLS_e ||
                m_anm_heap_under[UNDER_MOVE0_e].mIdx == dRes_INDEX_LKANM_BCK_ATNRS_e)
            {
...
void daPy_lk_c::setFrontWallType() {
    f32 radius = mAcchCir[0].GetWallR();
    if (mFrontWallType != 0) {
        return;
    }
    mFrontWallType = 1;
    ...
    int i;
    for (i = 2; i >= 0; i--) {
        start.set(current.pos.x, current.pos.y + mAcchCir[i].GetWallH(), current.pos.z);
        end.set(start.x + (sin * (25.0f + radius)), start.y, start.z + (cos * (25.0f + radius)));
    ...
    cM3dGPla* wall_plane = dComIfG_Bgsp()->GetTriPla(mLinkLinChk);
    if (std::abs(wall_plane->GetNP()->y) > 0.05f) {
        return;
    }
    m352C = cM_atan2s(wall_plane->GetNP()->x, wall_plane->GetNP()->z);
    ...
    if (checkModeFlg(ModeFlg_MIDAIR)) {
        ...
        f32 dVar12 = cM3d_SignedLenPlaAndPos(wall_plane, &current.pos);
        m3724.set(
            current.pos.x - (dVar12 * wall_plane->GetNP()->x),
            current.pos.y,
            current.pos.z - (dVar12 * wall_plane->GetNP()->z)
        );
        int wall_code = dComIfG_Bgsp()->GetWallCode(mLinkLinChk);
        if (wall_code != 4 && wall_code != 5) {
            i = 2;
            dVar12 = 2.0f * dVar12;
            for (; i >= 0; i--) {
                start.set(current.pos.x, current.pos.y + ((mAcchCir[i]).GetWallH()), current.pos.z);
                end.set(start.x - (dVar12 * wall_plane->GetNP()->x), start.y, start.z - (dVar12 * wall_plane->GetNP()->z));
                mLinkLinChk.Set(&start, &end, this);
                if (dComIfG_Bgsp()->LineCross(&mLinkLinChk)) {
                    cXyz sp90 = *wall_plane->GetNP() - *dComIfG_Bgsp()->GetTriPla(mLinkLinChk)->GetNP();
                    if (sp90.abs() < 0.001f) {
                        break;
                    }
                }
    ...
    int wall_code = dComIfG_Bgsp()->GetWallCode(mLinkLinChk);
    if (wall_code == 2 || current.pos.y - m35D4 < 125.0f) {
        return;
    }
    if (checkNoResetFlg0(daPyFlg0_UNK100)) {
        if ((wall_code == 3 && !checkModeFlg(ModeFlg_SWIM)) && !checkModeFlg(ModeFlg_MIDAIR)) {
            ...
            if (wall_code == 1) {
                if (checkModeFlg(ModeFlg_MIDAIR)) {
                    start.set(
                        m3724.x - 15.0f * wall_plane->GetNP()->x,
                        150.0f + current.pos.y,
                        m3724.z - 15.0f * wall_plane->GetNP()->z
                    );
                    mGndChk.SetPos(&start);
                    if (dComIfG_Bgsp()->GroundCross(&mGndChk) >= current.pos.y) {
                        cM3dGPla* uVar7 = dComIfG_Bgsp()->GetTriPla(mGndChk);
                        if (cBgW_CheckBGround(uVar7->GetNP()->y)) {
                            cXyz sp84;
                            cXyz sp78(wall_plane->GetNP()->z, 0.0f, wall_plane->GetNP()->x);
                            sp78.normalize();
                            cM3dGPla spC8(&sp78, -(sp78.x * m3724.x + sp78.z * m3724.z));
                            uVar7 = dComIfG_Bgsp()->GetTriPla(mGndChk);
                            if (cM3d_3PlaneCrossPos(*wall_plane, *uVar7, spC8, &sp84) &&
            ...
            if (wall_code == 4 || (wall_code == 5 && !checkModeFlg(ModeFlg_MIDAIR))) {
                if (wall_code == 4) {
                    mFrontWallType = 4;
                } else {
                    mFrontWallType = 5;
    ...
    start.set(current.pos.x, current.pos.y + (0.1f + m_HIO->mWallCatch.m.field_0x18), current.pos.z);
    end.set(start.x + (sin * (50.0f + radius)), start.y, start.z + (cos * (50.0f + radius)));
    mLinkLinChk.Set(&start, &end, this);
    BOOL cVar8 = dComIfG_Bgsp()->LineCross(&mLinkLinChk);
    if (!cVar8) {
        mRoofChk.SetPos(current.pos);
        if ((dComIfG_Bgsp()->RoofChk(&mRoofChk) - current.pos.y) > 10.0f + m_HIO->mWallCatch.m.field_0x18) {
            cVar8 = FALSE;
        } else {
            cVar8 = TRUE;
        }
    ...
    if (cVar8) {
        if (!checkGrabAnime() && !checkModeFlg(ModeFlg_MIDAIR | ModeFlg_SWIM)) {
            start.set(current.pos.x, current.pos.y + 149.9f, current.pos.z);
            end.set(
                start.x + (2.0f * radius) * cM_ssin(current.angle.y),
                start.y,
                start.z + (2.0f * radius) * cM_scos(current.angle.y)
            );
            cXyz sp48;
            cM3dGPla* uVar7 = getWHideModePolygon(&start, &end, &sp48, 4);
            if (uVar7 != 0) {
                mFrontWallType = 2;
            }
        }
    ...
    if (!checkModeFlg(ModeFlg_SWIM) && ground_y < current.pos.y) {
        return;
    }
    if (ground_y == -G_CM3D_F_INF || !cBgW_CheckBGround(dComIfG_Bgsp()->GetTriPla(mGndChk)->GetNP()->y)) {
        return;
    }
    cXyz sp3C;
    cXyz sp30(wall_plane->GetNP()->z, 0.0f, -wall_plane->GetNP()->x);
    sp30.normalize();
    cM3dGPla spB4(&sp30, -(sp30.x * m3724.x + sp30.z * m3724.z));
    if (!cM3d_3PlaneCrossPos(*wall_plane, *dComIfG_Bgsp()->GetTriPla(mGndChk), spB4, &sp3C)) {
        return;
    }
    m3724 = sp3C;
    ...
    if (checkModeFlg(ModeFlg_SWIM)) {
        ...
        if (((ground_y - mAcch.GetGroundH()) > 125.0f) && (mCurProc != daPyProc_HOOKSHOT_FLY_e || fVar3 < 100.0f)) {
            mFrontWallType = 7;
        }
    ...
    if (checkModeFlg(ModeFlg_MIDAIR)) {
        ...
        if (!checkNoResetFlg1(daPyFlg1_VINE_CATCH)) {
            ...
            if (mFrontWallType == 4) {
                ...
                f32 fVar3 = cM_scos(m352C);
                int iVar8 = (int)((current.pos.y - m3724.y) / 37.5f) - 1;
                if (iVar8 < 1) {
        ...
        if (mFrontWallType == 4) {
            ...
            if (checkModeFlg(ModeFlg_SWIM)) {
                f32 fVar3 = cM_ssin(m352C);
                f32 fVar2 = cM_scos(m352C);
                shape_angle.y = m352C + 0x8000;
                current.angle.y = shape_angle.y;
                int iVar8 = (int)((current.pos.y - m3724.y) / 37.5f) - 2;
...
#if VERSION == VERSION_DEMO
void daPy_lk_c::setDamagePoint(f32 amount) {
    if (!checkNoDamageMode()) {
...
BOOL daPy_lk_c::checkNormalDamage(int param_1) {
    if (checkGrabAnime() || mCurProc != daPyProc_MOVE_e ||
        (std::abs(mNormalSpeed * (m3580 == 8 ? 1.0f : cM_scos(m34E2)) / mMaxNormalSpeed) < 0.9f) ||
        (!checkNoDamageMode() && !dComIfGs_checkBottle(dItemNo_FAIRY_BOTTLE_e) && dComIfGs_getLife() <= param_1))
    {
        return true;
    }
    return false;
...
void daPy_lk_c::setDashDamage() {
    setActAnimeUpper(dRes_INDEX_LKANM_BCK_DAMDASH_e, UPPER_MOVE2_e, m_HIO->mMove.m.field_0x48, 0.0f, -1,
                     m_HIO->mBasic.m.field_0xC);
    ...
    if (checkModeFlg(ModeFlg_04000000)) {
        ...
        if (mCyl.ChkTgHit() && !checkGuardSlip() &&
            ...
            if (!checkSuccessGuard(spl)) {
                ...
                if (checkGrabWear() &&
                    mCyl.GetTgHitAc() && fopAcM_GetName(mCyl.GetTgHitAc()) == fpcNm_NZ_e &&
                    grab_actor != NULL
                ) {
        ...
        !mAcch.ChkGroundHit())
    {
        ...
        if (dVar11 < -30.1f) {
            ...
            if (dVar10 > 0.001f) {
                ...
                dVar10 = mNormalSpeed;
                if (dComIfGs_getLife() != 0 || dComIfGs_checkBottle(dItemNo_FAIRY_BOTTLE_e)) {
...
BOOL daPy_lk_c::changeDemoProc() {
    ...
    JUT_ASSERT(VERSION_SELECT(9275, 9293, 9342, 9342), (demo_mode < daPy_demo_c::DEMO_LAST_e) || (demo_mode == daPy_demo_c::DEMO_NEW_ANM0_e));
    ...
    if (demo_mode == daPy_demo_c::DEMO_BJUMP_e) {
        onNoResetFlg0(daPyFlg0_UNK100000);
    ...
    if (demo_mode == daPy_demo_c::DEMO_PFALL_e) {
        onNoResetFlg0(daPyFlg0_UNK100000);
        return procFall_init(1, m_HIO->mWallCatch.m.field_0x54);
    }
    if (demo_mode == daPy_demo_c::DEMO_SFALL_e) {
        if (mCurProc == daPyProc_LAND_e) {
            return true;
        }
        return procSlowFall_init();
    }
    if (demo_mode == daPy_demo_c::DEMO_LWAIT_e) {
        if ((!checkModeFlg(
    ...
    if (demo_mode == daPy_demo_c::DEMO_TACT_e) {
        onNoResetFlg0(daPyFlg0_UNK100000);
    ...
    if (demo_mode == daPy_demo_c::DEMO_UNK_051_e) {
        if (mActorKeepGrab.getActor() == NULL || mActorKeepGrab.getActor() != dComIfGp_getCb1Player()) {
            deleteEquipItem(FALSE);
            freeGrabItem();
    ...
    if (demo_mode == daPy_demo_c::DEMO_BO_OPEN_e) {
        onNoResetFlg0(daPyFlg0_UNK100000);
        return procBottleOpen_init(dComIfGp_event_getPreItemNo());
    }
    if (((checkNoResetFlg0(daPyFlg0_UNK100000) || mCurProc == daPyProc_SUBJECTIVITY_e) ||
                checkUpperReadyThrowAnime()) &&
...
BOOL daPy_lk_c::changeDeadProc() {
    if (!dComIfGp_event_runCheck() && !checkPlayerDemoMode() && dComIfGs_getLife() == 0 &&
        !checkModeFlg(ModeFlg_DAMAGE))
    {
        if (dComIfGs_checkBottle(dItemNo_FAIRY_BOTTLE_e)) {
            makeFairy(&current.pos, daNpc_Fa1_c::Type_LINK_DOWN_e);
            dComIfGs_setBottleItemIn(dItemNo_FAIRY_BOTTLE_e, dItemNo_EMPTY_BOTTLE_e);
            return false;
        }
...
BOOL daPy_lk_c::checkBottleItem(int itemNo) const {
    return itemNo >= dItemNo_EMPTY_BOTTLE_e && itemNo <= dItemNo_UNK_BOTTLE_60_e;
}

/* 80111F7C-80111FEC       .text checkDrinkBottleItem__9daPy_lk_cCFi */
BOOL daPy_lk_c::checkDrinkBottleItem(int itemNo) const {
    return itemNo == dItemNo_RED_POTION_e || itemNo == dItemNo_GREEN_POTION_e || itemNo == dItemNo_BLUE_POTION_e || itemNo == dItemNo_SOUP_BOTTLE_e || itemNo == dItemNo_HALF_SOUP_BOTTLE_e;
}

/* 80111FEC-8011201C       .text checkOpenBottleItem__9daPy_lk_cCFi */
BOOL daPy_lk_c::checkOpenBottleItem(int itemNo) const {
    return itemNo == dItemNo_FIREFLY_BOTTLE_e || itemNo == dItemNo_WATER_BOTTLE_e || itemNo == dItemNo_FOREST_WATER_e || itemNo == dItemNo_FAIRY_BOTTLE_e;
}

/* 8011201C-80112044       .text checkBowItem__9daPy_lk_cCFi */
BOOL daPy_lk_c::checkBowItem(int itemNo) const {
    return itemNo == dItemNo_BOW_e || itemNo == dItemNo_MAGIC_ARROW_e || itemNo == dItemNo_LIGHT_ARROW_e;
}

/* 80112044-80112064       .text checkPhotoBoxItem__9daPy_lk_cCFi */
BOOL daPy_lk_c::checkPhotoBoxItem(int itemNo) const {
    return itemNo == dItemNo_PICTO_BOX_e || itemNo == dItemNo_DELUXE_PICTO_BOX_e;
}
...
BOOL daPy_lk_c::commonProcInit(daPy_PROC proc) {
    ...
    if (mCurProc == daPyProc_SLIP_e) {
        ...
        setSmallFanModel();
        mEquipItem = dItemNo_DEKU_LEAF_e;
        m35F0 = m3688.y;
        m3730 = cXyz::Zero;
        m34E0 = 0;
        m34E4 = 0;
        ...
        if (mCurProc == daPyProc_DEMO_GET_ITEM_e && mProcVar4.m34D8 != 0) {
            m34C2 = 0xB;
        }
        ...
        if (item && (fopAcM_GetName(item) == fpcNm_ITEM_e || fopAcM_GetName(item) == fpcNm_Demo_Item_e)) {
            item->dead();
        }
    ...
    if (mEquipItem == dItemNo_SKULL_HAMMER_e) {
        mSwordAnim.changeBckOnly(getItemAnimeResource(dRes_INDEX_LKANM_BCK_HAMMERDAM_e));
    } else if (checkBowItem(mEquipItem) && !checkBowAnime()) {
        mSwordAnim.changeBckOnly(getItemAnimeResource(dRes_INDEX_LKANM_BCK_ARROWRELORDA_e));
    }
...
BOOL daPy_lk_c::procSubjectivity() {
    ...
    if(
        ...
        (mProcVar6.m3570 != 0 && !spActionButton() // Player released R while crouching
    )) {
        seStartSystem(JA_SE_SUBJ_VIEW_OUT);
...
BOOL daPy_lk_c::procCrouchDefense_init() {
    ...
    daPy_matAnm_c::offMabaFlg();
    setSingleMoveAnime(ANM_DIFENCE, m_HIO->mCrouch.m.field_0x10, 0.0f, -1, m_HIO->mCrouch.m.field_0x14);
    current.angle.y = shape_angle.y;
    mProcVar2.m34D4 = 0;
...
BOOL daPy_lk_c::procCrouchDefenseSlip_init() {
                             ...
                             tgHitObj->GetAtSpl() == dCcG_At_Spl_UNK7))
    {
        setSingleMoveAnime(
            ANM_DIFENCEA,
            m_HIO->mNockback.m.field_0x8,
            m_HIO->mNockback.m.field_0xC,
            m_HIO->mNockback.m.field_0x0,
            m_HIO->mNockback.m.field_0x10
        );
        dVar5 = damage_vec->absXZ();
        mNormalSpeed = (m_HIO->mNockback.m.field_0x18 * dVar5) + m_HIO->mNockback.m.field_0x14;
        ...
        dVar5 = damage_vec->absXZ();
        mNormalSpeed = (m_HIO->mGuard.m.field_0x14 * dVar5) + m_HIO->mGuard.m.field_0x10;
...
BOOL daPy_lk_c::procCrouchDefenseSlip() {
    ...
    f32 dVar2 = cLib_addCalc(&mNormalSpeed, 0.0f, m35A0, m35A4, m35A8);
    if (dVar2 <= 0.001f) {
        if (mProcVar6.m3570 != 0) {
...
BOOL daPy_lk_c::procWaitTurn_init() {
    ...
    setSingleMoveAnime(ANM_ROT, m_HIO->mBasic.m.field_0x4, 0.0f, -1, m_HIO->mBasic.m.field_0xC);
    if (dComIfGp_event_runCheck()) {
        mNormalSpeed = 0.0f;
    }
...
BOOL daPy_lk_c::procMoveTurn_init(int param_1) {
    ...
    if (param_1 != 0) {
        mProcVar2.m34D4 = (m_HIO->mMove.m.field_0x0 * 4) + 0x4A56;
        mProcVar3.m34D6 = m_HIO->mMove.m.field_0x0 * 2;
        mProcVar0.m34D0 = 2;
        current.angle.y = m34E8;
        mNormalSpeed *= 0.5f;
    } else {
        mProcVar2.m34D4 = m_HIO->mMove.m.field_0x0 * 2;
        mProcVar3.m34D6 = m_HIO->mMove.m.field_0x0;
        mProcVar0.m34D0 = 3;
    }
    return true;
}
...
BOOL daPy_lk_c::procMoveTurn() {
    setSpeedAndAngleNormal(m_HIO->mMove.m.field_0x0);
    cLib_addCalcAngleS(&shape_angle.y, current.angle.y, mProcVar0.m34D0, mProcVar2.m34D4, mProcVar3.m34D6);
    if (!checkNextMode(0)) {
        setBlendMoveAnime(-1.0f);
    }
    return true;
...
BOOL daPy_lk_c::procSlip_init() {
    ...
    daPy_matAnm_c::offMabaFlg();
    setSingleMoveAnime(ANM_SLIP, m_HIO->mSlip.m.field_0xC, 0.0f, -1, m_HIO->mSlip.m.field_0x1C);
    mNormalSpeed = speedF * m_HIO->mSlip.m.field_0x8;
    if (m_HIO->mSlip.m.field_0x0 == 1) {
        mNormalSpeed = mMaxNormalSpeed;
    } else if (m_HIO->mSlip.m.field_0x0 == 2 && mNormalSpeed > mMaxNormalSpeed) {
        mNormalSpeed = mMaxNormalSpeed;
    }
...
BOOL daPy_lk_c::procSlip() {
    f32 dVar5 = cLib_addCalc(&mNormalSpeed, 0.0f, m_HIO->mSlip.m.field_0x18,
                             m_HIO->mSlip.m.field_0x10, m_HIO->mSlip.m.field_0x14);
    if (dVar5 <= 0.001f) {
        if (mStickDistance > 0.05f) {
            current.angle.y = shape_angle.y + 0x8000;
            shape_angle.y = shape_angle.y + 0x100;
            mNormalSpeed = mMaxNormalSpeed * 0.5f;
            procMoveTurn_init(0);
        } else {
            checkNextMode(0);
        }
        ...
        if (mNormalSpeed < m_HIO->mSlip.m.field_0x20) {
            resetFootEffect();
        }
        cXyz local_1c;
        cXyz local_28;
...
BOOL daPy_lk_c::procSlideFront_init(s16 param_1) {
    ...
    setSingleMoveAnime(ANM_SLIDEF, m_HIO->mSlide.m.field_0xC, m_HIO->mSlide.m.field_0x10, m_HIO->mSlide.m.field_0x0, m_HIO->mSlide.m.field_0x14);
...
BOOL daPy_lk_c::procSlideFront() {
    ...
    if (pfVar2 != NULL) {
        ...
        mNormalSpeed += (1.0f + 0.5f * (1.0f - pfVar2->GetNP()->y)) * m_HIO->mSlide.m.field_0x48 *
...
BOOL daPy_lk_c::procSlideBack_init(s16 param_1) {
    ...
    m3526 = 8;
    current.angle.y = param_1;
    setSingleMoveAnime(ANM_SLIDEB, m_HIO->mSlide.m.field_0x18, m_HIO->mSlide.m.field_0x1C, m_HIO->mSlide.m.field_0x2, m_HIO->mSlide.m.field_0x20);
    mMaxNormalSpeed = m_HIO->mSlide.m.field_0x44;
    setFootEffectPosType(3);
...
BOOL daPy_lk_c::procSlideFrontLand_init() {
    ...
    setSingleMoveAnime(ANM_SLIDEFLAND, m_HIO->mSlide.m.field_0x24, m_HIO->mSlide.m.field_0x28, m_HIO->mSlide.m.field_0x4, m_HIO->mSlide.m.field_0x30);
    mNormalSpeed *= 0.5f;
    return true;
}
...
BOOL daPy_lk_c::procSlideFrontLand() {
    cLib_addCalc(&mNormalSpeed, 0.0f, 0.5f, 5.0f, 1.0f);
    if (mFrameCtrlUnder[UNDER_MOVE0_e].getRate() < 0.01f) {
        current.angle.y = shape_angle.y;
        checkNextMode(0);
    } else {
...
BOOL daPy_lk_c::procSlideBackLand_init() {
    ...
    setSingleMoveAnime(ANM_SLIDEBLAND, m_HIO->mSlide.m.field_0x34, m_HIO->mSlide.m.field_0x38, m_HIO->mSlide.m.field_0x6, m_HIO->mSlide.m.field_0x40);
    return true;
}
...
BOOL daPy_lk_c::procFrontRollCrash_init() {
    ...
    if (mAcch.ChkWallHit() && mAcchCir[0].ChkWallHit()) {
        ...
        if (actor != 0 && fopAcM_GetName(actor) == fpcNm_Obj_Movebox_e) {
...
BOOL daPy_lk_c::procAutoJump_init() {
    ...
    m35F0 = m3688.y;
    m35F4 = m3688.y;
    setSingleMoveAnime(ANM_JMPST, m_HIO->mAutoJump.m.field_0x20, m_HIO->mAutoJump.m.field_0x2C, m_HIO->mAutoJump.m.field_0x2, m_HIO->mAutoJump.m.field_0x30);
...
BOOL daPy_lk_c::procAutoJump() {
    if (mProcVar0.m34D0 == 0) {
        if (mStickDistance > 0.05f && getDirectionFromCurrentAngle() == DIR_BACKWARD) {
            cLib_chaseF(&mNormalSpeed, 0.0f, (m_HIO->mAutoJump.m.field_0x3C * mStickDistance));
        } else if (mStickDistance <= 0.05f) {
            cLib_chaseF(&mNormalSpeed, 0.0f, (0.1f * m_HIO->mAutoJump.m.field_0x3C));
        } else {
            cLib_chaseF(&mNormalSpeed, m_HIO->mMove.m.field_0x18,
                        m_HIO->mAutoJump.m.field_0x1C);
        }
    } else {
        cLib_chaseF(&mNormalSpeed, m_HIO->mMove.m.field_0x18, m_HIO->mAutoJump.m.field_0x1C);
    }
    checkNextActionItemFly();
    ...
    if (mProcVar0.m34D0 > 0) {
        ...
        if (doTrigger() || mProcVar1.m34D2 == 0) {
    ...
    } else if (speed.y < -gravity && mProcVar6.m3570 != 2) {
        if (mActorKeepGrab.getActor() != NULL && fopAcM_GetName(mActorKeepGrab.getActor()) == fpcNm_NPC_MD_e &&
            dComIfGs_isEventBit(dSv_event_flag_c::UNK_1620))
        {
            speed.y = 5.0f;
            gravity = 0.0f;
            mProcVar0.m34D0 = m_HIO->mAutoJump.m.field_0x8;
        }
...
BOOL daPy_lk_c::procLandDamage_init(int param_1) {
    ...
    mNormalSpeed = 0.0f;
    if (param_1 == 0) {
        mProcVar0.m34D0 = m_HIO->mFall.m.field_0xC;
        offModeFlg(ModeFlg_DAMAGE);
    } else {
        mProcVar0.m34D0 = m_HIO->mFall.m.field_0x2;
        mDamageWaitTimer = m_HIO->mDam.m.field_0x0;
    }
    setSingleMoveAnime(ANM_LANDDAMA, m_HIO->mFall.m.field_0x1C, m_HIO->mFall.m.field_0x20, m_HIO->mFall.m.field_0x4, m_HIO->mFall.m.field_0x24);
    mProcVar6.m3570 = 0;
...
BOOL daPy_lk_c::procFall() {
    ...
    if (mProcVar3.m34D6 != 0) {
        cLib_addCalc(&mNormalSpeed, 0.0f, 0.2f, 1.0f, 0.1f);
    }
    checkNextActionItemFly();
    if (checkJumpFlower() || (mProcVar7.m3574 != 0 && checkJumpRideShip())) {
        return true;
    }
    if (mAcch.ChkGroundHit()) {
        ...
        if (mProcVar0.m34D0 == 1) {
            ...
            if (mStickDistance > 0.05f && getDirectionFromShapeAngle() == 0 && mProcVar1.m34D2 == 0) {
                if (changeFrontWallTypeProc()) {
                    return true;
                }
            }
        } else if (mProcVar0.m34D0 == 2) {
...
BOOL daPy_lk_c::procSmallJump_init(int param_1) {
    ...
    setSingleMoveAnime(ANM_JMPST, m_HIO->mSmallJump.m.field_0x4, m_HIO->mSmallJump.m.field_0x8, m_HIO->mSmallJump.m.field_0x0, m_HIO->mSmallJump.m.field_0xC);
    if (param_1 == 0) {
        mNormalSpeed = m_HIO->mSmallJump.m.field_0x10;
        speed.y = std::sqrtf(
            -gravity * ((m3724.y - current.pos.y) + m_HIO->mSmallJump.m.field_0x14) * 2.0f);
        mProcVar6.m3570 = 2;
    } else {
        mNormalSpeed = 2.0f * m_HIO->mSmallJump.m.field_0x10;
        speed.y = 20.0f;
        mProcVar6.m3570 = 0;
    }
    if (checkHeavyStateOn()) {
...
BOOL daPy_lk_c::procSmallJump() {
    if (mAcch.ChkGroundHit()) {
        ...
        procFall_init(mProcVar6.m3570, m_HIO->mSmallJump.m.field_0x18);
        setTextureAnime(0x37, 0);
    }
    return true;
}
...
BOOL daPy_lk_c::procVerticalJump_init() {
    ...
    setSingleMoveAnime(ANM_VJMP, m_HIO->mWallCatch.m.field_0x3C, 0.0f, m_HIO->mWallCatch.m.field_0x6, m_HIO->mWallCatch.m.field_0x40);
    mProcVar0.m34D0 = 0;
...
BOOL daPy_lk_c::procVerticalJump() {
    ...
    cLib_addCalcAngleS(&shape_angle.y, mProcVar2.m34D4, 2, 0x1000, 0x400);
    current.angle.y = shape_angle.y;
    if (mFrameCtrlUnder[UNDER_MOVE0_e].getRate() < 0.01f) {
...
BOOL daPy_lk_c::procGuardCrash() {
    cLib_addCalc(&mNormalSpeed, 0.0f, 0.5f, 1.25f, 0.25f);
    if (mProcVar0.m34D0 > 0) {
        mProcVar0.m34D0--;
    } else {
        checkNextMode(0);
...
BOOL daPy_lk_c::procDamage_init() {
    ...
    mProcVar2.m34D4 = cM_atan2s(local_54.z, local_54.y);
...
BOOL daPy_lk_c::procDamage() {
    ...
    m3564.x = mProcVar2.m34D4 * fVar1;
    m3564.y = 0;
    m3564.z = mProcVar3.m34D6 * fVar1;
    mBodyAngle.x = mProcVar2.m34D4 * fVar2;
    mBodyAngle.z = mProcVar3.m34D6 * fVar2;
    mBodyAngle.y = 0;
    dVar5 = cLib_addCalc(&mNormalSpeed, 0.0f, m_HIO->mDam.mDamage.m.field_0x1C,
    ...
    if (dVar5 <= 0.001f && mFrameCtrlUnder[UNDER_MOVE0_e].getRate() < 0.01f) {
        checkNextMode(0);
    } else if (mNormalSpeed < m_HIO->mDam.mDamage.m.field_0x2C) {
        resetFootEffect();
    }
    return true;
}
...
BOOL daPy_lk_c::procPolyDamage_init() {
    ...
    setSingleMoveAnime(
        ANM_DAM,
        m_HIO->mDam.mDamage.m.field_0x30,
        m_HIO->mDam.mDamage.m.field_0x34,
        m_HIO->mDam.mDamage.m.field_0x4,
        m_HIO->mDam.mDamage.m.field_0x38
    );
    mNormalSpeed = 0.0f;
    voiceStart(2);
    seStartOnlyReverb(JA_SE_LK_DAMAGE_NORMAL);
...
BOOL daPy_lk_c::procLargeDamage_init(int param_1, int param_2, s16 param_3, s16 param_4) {
    ...
    if (param_1 == -5) {
        ...
        {
            m32E4.makeEmitter(dPa_name::ID_AK_SN_BKMHAKIDASHIHOUSHI00, mpCLModel->getAnmMtx(CL_JNT_CENTER_e), &current.pos, NULL);
        }
        ...
        iVar7 = 0;
        current.angle.y = shape_angle.y;
        f32 fVar2 = ((2.0f * m_HIO->mDam.mHuDamage.m.field_0x20) / -m_HIO->mDam.mHuDamage.m.field_0x1C) *
    ...
    if (param_1 < 0) {
        ...
        if (param_1 != -5) {
            if (mProcVar0.m34D0 > 0) {
                ...
                if (param_2 != 0) {
                    mNormalSpeed = m_HIO->mDam.mLaDamage.m.field_0x18;
                    speed.y = m_HIO->mDam.mLaDamage.m.field_0x20;
                } else {
                    mNormalSpeed = m_HIO->mDam.mHuDamage.m.field_0x18;
        ...
        if (param_2 != 0) {
            mNormalSpeed = m_HIO->mDam.mLaDamage.m.field_0x64;
        } else {
            mNormalSpeed = m_HIO->mDam.mHuDamage.m.field_0x64;
        }
    ...
    mBodyAngle.z = 0;
    if (param_1 == -7) {
        mProcVar3.m34D6 = mProcVar3.m34D6 | 2;
        mProcVar6.m3570 = -4;
    }
    return true;
}
...
BOOL daPy_lk_c::procLargeDamage() {
    ...
    s16* psVar3;
    if ((mProcVar3.m34D6 & 4)) {
        psVar3 = &m34F2;
    } else {
        psVar3 = &m34F4;
    }
    cLib_chaseAngleS(psVar3, mProcVar2.m34D4, mProcVar4.m34D8);
    if ((mProcVar3.m34D6 & 1)) {
        if (mAcch.ChkGroundHit()) {
            if (changeSlideProc()) {
                mNormalSpeed = 0.0f;
            } else {
                if ((mProcVar3.m34D6 & 2)) {
                    voiceStart(2);
                    seStartSystem(JA_SE_MAJUTOU_JAIL_DOOR);
                    mAcch.ClrWallNone();
                    mAcch.OffLineCheckNone();
                }
                procLargeDamageUp_init(mProcVar6.m3570, (u32)mProcVar3.m34D6 >> 3 & 1, m34F2, m34F4);
            }
        } else if (mAcch.ChkWallHit() && mNormalSpeed > m35A0) {
            ...
            for (int i = 0; i < 3; i++, pdVar7++) {
                if (pdVar7->ChkWallHit()) {
                    ...
                    mLinkLinChk.Set(&local_1c, &local_28, this);
                    if (dComIfG_Bgsp()->LineCross(&mLinkLinChk)) {
                        return procLargeDamageWall_init(mProcVar6.m3570, (u32)mProcVar3.m34D6 >> 3 & 1, m34F2,
                                                        m34F4);
                        break;
                    }
                }
        ...
        mProcVar3.m34D6 = mProcVar3.m34D6 | 1;
    }
    return true;
}
...
BOOL daPy_lk_c::procLargeDamageWall() {
    if (mFrameCtrlUnder[UNDER_MOVE0_e].getRate() < 0.01f) {
        procLargeDamage_init(mProcVar6.m3570, mProcVar0.m34D0, m34F2, m34F4);
    }
    return true;
}
...
BOOL daPy_lk_c::procLavaDamage_init() {
    ...
    gravity = -2.5f;
    mNormalSpeed = 15.0f + (40.0f * dVar3);
    speed.y = 32.0f + (36.0f * dVar3);
    setSingleMoveAnime(ANM_LAVADAM, m_HIO->mRestart.m.field_0x0, 0.0f, -1, m_HIO->mRestart.m.field_0x4);
    m32E4.makeEmitter(dPa_name::ID_AK_SN_HIDARUMAFIRE, mpCLModel->getAnmMtx(CL_JNT_LINK_ROOT_e), &current.pos, NULL);
    voiceStart(4);
...
BOOL daPy_lk_c::procElecDamage_init(const cXyz* param_1) {
    ...
    mNormalSpeed = 0.0f;
    speed.y = 0.0f;
    setSingleMoveAnime(ANM_DAMBIRI, m_HIO->mDam.mElecDamage.m.field_0x4, 0.0f, -1, m_HIO->mDam.mElecDamage.m.field_0x8);
    mDamageWaitTimer = m_HIO->mDam.m.field_0x0;
    mProcVar0.m34D0 = m_HIO->mDam.mElecDamage.m.field_0x0;
    if (param_1 != 0) {
...
BOOL daPy_lk_c::procElecDamage() {
    ...
    if (mProcVar0.m34D0 > 0) {
        ...
        endDemoMode();
        if (mAcch.ChkGroundHit() && mProcVar3.m34D6 == 0) {
            checkNextMode(0);
        } else {
            if (mProcVar3.m34D6 != 0) {
...
BOOL daPy_lk_c::procGuardSlip() {
    ...
    if (cLib_addCalc(&mNormalSpeed, 0.0f, m35A0, m35A4, m35A8) <= 0.001f) {
        if (mProcVar6.m3570 != 0) {
            return procNockBackEnd_init();
        }
        if (mProcVar0.m34D0 == 0) {
...
BOOL daPy_lk_c::procIceSlipAlmostFall_init() {
    ...
    setSingleMoveAnime(anm, m_HIO->mIceSlip.m.field_0x64, m_HIO->mIceSlip.m.field_0x68, m_HIO->mIceSlip.m.field_0xC, m_HIO->mIceSlip.m.field_0x70);
    setTextureAnime(6, 0);
    mNormalSpeed = 0.0f;
    return true;
}
...
BOOL daPy_lk_c::procBootsEquip_init(u16 param_1) {
    ...
    setSingleMoveAnime(
        ANM_SETBOOTS,
        m_HIO->mItem.mBoots.m.field_0x4,
        m_HIO->mItem.mBoots.m.field_0x8,
        m_HIO->mItem.mBoots.m.field_0x0,
        m_HIO->mItem.mBoots.m.field_0x10
    );
    mProcVar6.m3570 = param_1;
    return true;
}
...
BOOL daPy_lk_c::procNotUse_init(int param_1) {
    ...
    if (isDaizaItem(mProcVar6.m3570)) {
        m3624 = 0xf0c;
    } else {
        m3624 = 0x835;
    }
...
BOOL daPy_lk_c::procNotUse() {
    if (mProcVar2.m34D4 == 0 && !checkBottleItem((u16)mProcVar6.mBottleItem)) {
        dComIfGp_event_setItemPartnerId(fopAcM_createItemForPresentDemo(
            &current.pos,
            mProcVar6.mBottleItem,
    ...
    if (mFrameCtrlUnder[UNDER_MOVE0_e].getRate() < 0.01f) {
        daItemBase_c* item = (daItemBase_c*)fopAcM_getItemEventPartner(this);
        if (item != NULL && (fopAcM_GetName(item) == fpcNm_ITEM_e || fopAcM_GetName(item) == fpcNm_Demo_Item_e)) {
            item->show();
        }
        if (mProcVar2.m34D4 == 0 && checkBottleItem((u16)mProcVar6.mBottleItem)) {
...
void daPy_lk_c::setWaterY() {
    if (!daSea_ChkArea(current.pos.x, current.pos.z)) {
        ...
        mWaterY = mAcch.m_wtr.GetHeight();
        dPa_control_c::offStatus(1);
    } else {
        onNoResetFlg0(daPyFlg0_UNK80);
        mWaterY = daSea_calcWave(current.pos.x, current.pos.z);
        if (mAcch.m_wtr.GetHeight() > mWaterY) {
            mWaterY = mAcch.m_wtr.GetHeight();
            dPa_control_c::offStatus(1);
        } else {
...
void daPy_lk_c::setNeckAngle() {
    ...
    MtxP r25;
    r25 = mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e);
    cXyz* sp18;
    sp18 = NULL;
    s16 r30;
    s16 r29;
    BOOL r28;
    r28 = false;
    mDoMtx_multVecSR(mpCLModel->getAnmMtx(CL_JNT_CENTER_e), &cXyz::BaseZ, &sp94);
    ...
    if (dComIfGp_checkPlayerStatus1(0, daPyStts1_WIND_WAKER_CONDUCT_e)) {
        mDoMtx_multVec(mpCLModel->getAnmMtx(CL_JNT_CL_LHANDA_e), &l_tact_top, &spA0);
        sp18 = &spA0;
        r28 = true;
        ...
        if (mProcVar2.m34D4 == 0 && dComIfGp_getShipActor() != NULL) {
            sp18 = dComIfGp_getShipActor()->getCraneTop();
            r28 = true;
        }
    } else if (m_tex_anm_heap.mIdx == dRes_INDEX_LKANM_BTP_TDASHKAZE_e) {
        ...
        if (!dComIfGp_event_runCheck() || mDemo.getDemoMode() == daPy_demo_c::DEMO_RD_STOP_e) {
            ...
            if (mActorKeepRope.getActor() != NULL) {
                sp18 = &mActorKeepRope.getActor()->eyePos;
                r28 = true;
            }
        } else {
            ...
            if (
                ...
                fopAc_ac_c* r24_3 = getDemoLookActor();
                if (r24_3 != NULL) {
                    sp18 = &r24_3->eyePos;
                    r28 = true;
                }
    ...
    s16 r4;
    s16 r23;
    if (checkModeFlg(ModeFlg_00000080 | ModeFlg_08000000) && sp18 != NULL && !checkUpperAnime(dRes_INDEX_LKANM_BCK_DAMDASH_e)) {
        spB8 = *sp18 - spC4;
        ...
        if (checkModeFlg(ModeFlg_00000080) && m_anm_heap_upper[UPPER_MOVE0_e].mIdx != dRes_INDEX_LKANM_BCK_DASHKAZE_e &&
            m_anm_heap_upper[UPPER_MOVE1_e].mIdx != dRes_INDEX_LKANM_BCK_DASHKAZE_e)
        {
            if (r28) {
                r30 = (r27 >> 1) - r24_4;
                r29 = (r23_3 >> 1) - r25_3;
               ...
               mDirection == DIR_FORWARD)
    {
        ...
        if (abs(mProcVar2.m34D4) <= 0x4000) {
            r4 = -0x1800 * cM_scos(mProcVar2.m34D4);
        } else {
            r4 = 0;
        }
        r23 = 0x1800 * cM_ssin(mProcVar2.m34D4);
         ...
         m35A0 != 0.0f))
    {
        ...
        if (r28) {
            ...
            f0 = -cM_scos(mProcVar2.m34D4);
            f1 = cM_ssin(mProcVar2.m34D4);
...
void daPy_lk_c::checkOriginalHatAnimation() {
    if (
        ...
        onResetFlg0(daPyRFlg0_ORIGINAL_HAT_ANIM);
    }
}
#endif
...
void daPy_lk_c::setHatAngle() {
    ...
    MtxP head_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e);
    ...
    f32 f30;
    f30 = cM_scos(r3_3);
    MtxP hat_a_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_HATA_JNT_e);
    ...
    mDoMtx_multVecSR(mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e), &l_neck_front, &sp88);
    mDoMtx_multVecSR(mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e), &l_neck_top, &sp7C);
        ...
        if (mFanLightCps.ChkAtHit()) {
            ...
            if (m3438.getEmitter() != NULL) {
                if (mFanLightCps.GetAtHitAc() != 0 &&
                    (fopAcM_GetName(mFanLightCps.GetAtHitAc()) == fpcNm_BPW_e))
                {
...
void daPy_lk_c::setCollision() {
    MtxP root_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_LINK_ROOT_e);
    MtxP ltoe_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_LTOE_JNT_e);
    MtxP rtoe_jnt_mtx = mpCLModel->getAnmMtx(CL_JNT_RTOE_JNT_e);
    cXyz spD0;
    spD0.x = 0.5f * (root_jnt_mtx[0][3] + mpCLModel->getAnmMtx(CL_JNT_NECK_JNT_e)[0][3]);
    spD0.z = 0.5f * (root_jnt_mtx[2][3] + mpCLModel->getAnmMtx(CL_JNT_NECK_JNT_e)[2][3]);
    ...
    if (checkModeFlg(ModeFlg_CRAWL)) {
        ...
        spD0.y = root_jnt_mtx[1][3];
        mCyl.SetH(125.0f - (spD0.y - current.pos.y));
        mCyl.SetR(22.5f);
    } else {
        if (mCurProc == daPyProc_ROPE_SWING_e) {
            spD0.y = (ltoe_jnt_mtx[1][3] > root_jnt_mtx[1][3]) ? root_jnt_mtx[1][3] : ltoe_jnt_mtx[1][3];
        } else {
            spD0.y = (ltoe_jnt_mtx[1][3] > rtoe_jnt_mtx[1][3]) ? rtoe_jnt_mtx[1][3] : ltoe_jnt_mtx[1][3];
        }
        mCyl.SetH(40.1f + (mpCLModel->getAnmMtx(CL_JNT_NECK_JNT_e)[1][3] - spD0.y));
    }
    mCyl.SetC(spD0);
    ...
    if (checkResetFlg0(daPyRFlg0_UNK2)) {
        if (checkResetFlg0(daPyRFlg0_UNK1)) {
            ...
            int r23 = 10.0f * mFrameCtrlUnder[UNDER_MOVE0_e].getFrame();
            if (mCurProc != daPyProc_CUT_F_e && mCurProc != daPyProc_BT_VERTICAL_JUMP_CUT_e &&
                mCurProc != daPyProc_FAN_SWING_e)
            {
                if (mCurProc == daPyProc_JUMP_CUT_LAND_e) {
                    ...
                    r23 = 2.5f * mFrameCtrlUnder[UNDER_MOVE0_e].getFrame();
                } else {
                    r23 = 10.0f * mFrameCtrlUnder[UNDER_MOVE0_e].getFrame();
                    if (mCurProc == daPyProc_WEAPON_SIDE_SWING_e) {
                        r23 -= 5;
                    }
                }
        ...
        if (mCurProc == daPyProc_BOTTLE_OPEN_e && mProcVar3.m34D6 != 0) {
            mAtCps[0].OnAtSetBit();
    ...
    if (m353A != 0) {
        if (mCurProc == daPyProc_FAN_GLIDE_e) {
            cXyz local_10c;
            mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_LINK_ROOT_e), &local_10c);
...
void daPy_lk_c::setDemoData() {
    ...
    if (mDemo.getDemoType() == daPy_demo_c::TYPE_START_e && !dComIfGp_evmng_checkStartDemo()) {
        mDemo.setSystemDemoType();
    }
    if (demo_actor_p != NULL) {
        demo_actor_p->setModel(mpCLModel);
        mStaffIdx = -1;
        if (mDemo.getDemoType() != daPy_demo_c::TYPE_TOOL_e) {
        ...
        if (!checkPlayerDemoMode() && dComIfGp_event_runCheck()) {
            mDemo.setSystemDemoType();
        }
        if (mStaffIdx != -1) {
            ...
            if (cut_name != NULL) {
                ...
                if (mDemo.getDemoType() == daPy_demo_c::TYPE_SYSTEM_e) {
                ...
                if (face_p != NULL) {
                    if (*face_p == 0) {
                        ...
                        changeTextureAnime(dRes_INDEX_LKANM_BTP_TMABAI_e, dRes_INDEX_LKANM_BTK_TMABA_e, -1);
                    }
                }
            }
        }
    ...
    if (mDemo.getDemoType() == daPy_demo_c::TYPE_START_e) {
    ...
    if (mDemo.getDemoType() == daPy_demo_c::TYPE_TOOL_e || (mDemo.getDemoType() == daPy_demo_c::TYPE_SYSTEM_e && mStaffIdx != -1)) {
        if (prm0_p != NULL) {
        ...
        if ((demo_mode == daPy_demo_c::DEMO_INIT_WAIT_e) || (demo_mode == daPy_demo_c::DEMO_POS_INIT_e)) {
            ...
            f32 temp = std::abs(mNormalSpeed) / mMaxNormalSpeed;
            if (temp < m_HIO->mMove.m.field_0x2C) {
    ...
    } else if (mDemo.getDemoType() != daPy_demo_c::TYPE_ORIGINAL_e) {
        mDemo.setDemoMode(demo_mode);
    }
...
void daPy_lk_c::checkFallCode() {
    ...
    if (m3580 == 4 || -G_CM3D_F_INF == mAcch.GetGroundH()) {
        BOOL bVar4 = mWaterY > mAcch.GetGroundH() ? TRUE : FALSE;
        if (((!bVar4 && checkModeFlg(ModeFlg_MIDAIR) && !checkModeFlg(ModeFlg_HOOKSHOT) && m35F4 - current.pos.y > 100.0f * m_HIO->mFall.m.field_0x14) ||
    ...
    int iVar1 = dComIfG_Bgsp()->GetExitId(mAcch.m_gnd);
    s32 bVar2 = dComIfG_Bgsp()->GetRoomId(mAcch.m_gnd);
    daShip_c* ship = (daShip_c*)dComIfGp_getShipActor();
    if (iVar1 == 0x3f) {
        return;
    }
         ...
         (m3580 == 5 && m35F4 - current.pos.y > 500.0f)))
    {
        ...
        if (speedF < 1.5f) {
            dVar5 = 1.5f;
        } else if (speedF > m_HIO->mMove.m.field_0x18) {
...
void daPy_lk_c::setBeltConveyerPower() {
        ...
        (mAcch.ChkGroundHit() || checkModeFlg(ModeFlg_SWIM)))
    {
        ...
        cStack_3c *= local_58;
        f32 fVar1;
        if (m36B8.abs2XZ() > cStack_3c.abs2XZ()) {
            fVar1 = 3.0f;
        } else {
            fVar1 = 1.0f;
        }
...
void daPy_lk_c::setWindAtPower() {
    ...
    if (mWindCyl.ChkTgHit() && (mDamageWaitTimer == 0 || m34BA == 0) && !dComIfGp_event_runCheck() &&
        !checkPlayerDemoMode() && mCurProc != daPyProc_FAN_GLIDE_e)
    {
...
void daPy_lk_c::checkRoofRestart() {
    ...
    if (
        ...
        fopAcM_GetName(dComIfG_Bgsp()->GetActorPointer(mAcch.m_roof)) == fpcNm_BRIDGE_e
    ) {
        return;
    }
#endif

    f32 roof_height = mAcch.GetRoofHeight();
    if (roof_height > mAcch.GetGroundH()) {
        f32 dVar10 = mHeadTopPos.y - 10.0f;
        if (dVar10 < 10.0f + current.pos.y) {
            dVar10 = 10.0f + current.pos.y;
        }
        if (roof_height < dVar10) {
            if (checkModeFlg(ModeFlg_WHIDE)) {
                ...
                if (dComIfG_Bgsp()->RoofChk(&mRoofChk) >= dVar10) {
                    return;
                }
            }
...
BOOL daPy_lk_c::execute() {
    ...
    if (checkEquipHeavyBoots() &&
        dComIfGp_getSelectItem(dItemBtn_X_e) != dItemNo_IRON_BOOTS_e &&
        dComIfGp_getSelectItem(dItemBtn_Y_e) != dItemNo_IRON_BOOTS_e &&
        dComIfGp_getSelectItem(dItemBtn_Z_e) != dItemNo_IRON_BOOTS_e
    ) {
        offNoResetFlg0(daPyFlg0_EQUIP_HEAVY_BOOTS);
    } else if (checkEquipDragonShield() &&
        dComIfGp_getSelectItem(dItemBtn_X_e) != dItemNo_MAGIC_ARMOR_e &&
        dComIfGp_getSelectItem(dItemBtn_Y_e) != dItemNo_MAGIC_ARMOR_e &&
        dComIfGp_getSelectItem(dItemBtn_Z_e) != dItemNo_MAGIC_ARMOR_e
    ) {
        offNoResetFlg1(daPyFlg1_EQUIP_DRAGON_SHIELD);
    }
    ...
    if (checkModeFlg(ModeFlg_SWIM)) {
        ...
        current.pos = sp14;
        if (mProcVar7.m3574 != 0 && mAcch.GetGroundH() != -G_CM3D_F_INF) {
            current.pos.y = mAcch.GetGroundH();
        }
    ...
    mpCLModelData->getJointNodePointer(CL_JNT_WAIST_CHN_e)->setMtxCalc(m_pbCalc[PART_UNDER_e]);
    ...
    mpCLModel->calc();
    cMtx_multVec(mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e), &head_offset, &mHeadTopPos);
    checkRoofRestart();
    mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_CL_LHANDA_e), &mLeftHandPos);
    mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_CL_RHANDA_e), &mRightHandPos);
    cMtx_multVec(mpCLModel->getAnmMtx(CL_JNT_LINK_ROOT_e), &boomerang_catch, &mBoomerangCatchPos);
    cMtx_multVec(mpCLModel->getAnmMtx(CL_JNT_CL_LHANDA_e), &hookshot_root, &mHookshotRootPos);
    mpKatsuraModel->setBaseTRMtx(mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e));
    mpKatsuraModel->calc();
    mpYamuModel->setBaseTRMtx(mpCLModel->getAnmMtx(CL_JNT_HEAD_JNT_e));
    mpYamuModel->calc();
    setItemModel();
    
    if (mCurProc == daPyProc_CUT_F_e || mCurProc == daPyProc_BT_VERTICAL_JUMP_CUT_e) {
        if (checkNormalSwordEquip()) {
            mpSwordTipStabModel->setBaseTRMtx(mpEquipItemModel->getAnmMtx(CL_JNT_STOMACH_JNT_e));
        } else {
            mpSwordTipStabModel->setBaseTRMtx(mpEquipItemModel->getAnmMtx(CL_JNT_CHEST_JNT_e));
        }
    }
    
    cMtx_multVec(mpCLModel->getAnmMtx(CL_JNT_CL_EYE_e), &l_eye_offset, &eyePos);
    ...
    if (mEquipItem == daPyItem_BOKO_e
        ...
        if (checkNormalSwordEquip()) {
            cMtx_multVec(mpEquipItemModel->getAnmMtx(SWA_JNT_SWAROOT_JNT_e), &nsword_top, &mSwordTopPos);
            mDoMtx_multVecZero(mpEquipItemModel->getAnmMtx(SWA_JNT_SWAA_JNT_e), &m36C4);
        } else {
            mDoMtx_multVecZero(mpEquipItemModel->getAnmMtx(SHMS_JNT_SHAROOT_JNT_e), &m36C4);
            cMtx_multVec(mpEquipItemModel->getAnmMtx(SHMS_JNT_SHAROOT_JNT_e), &msword_top, &mSwordTopPos);
        }
    } else if (mCurProc == daPyProc_FAN_SWING_e) {
        mDoMtx_multVecZero(mpEquipItemModel->getBaseTRMtx(), &m36C4);
        cMtx_multVec(mpEquipItemModel->getAnmMtx(FAN_JNT_FANE_JNT_e), &fan_top, &mSwordTopPos);
    } else {
        mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_CL_PODA_e), &m36C4);
        mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_CL_PODA_e), &mSwordTopPos);
    }
    ...
    if (emitter) {
        emitter->setGlobalRTMatrix(mpCLModel->getAnmMtx(CL_JNT_LTOE_JNT_e));
    }
    ...
    if (daPy_dmEcallBack_c::getTimer() > 0) {
        ...
        if (daPy_dmEcallBack_c::checkFlame()) {
            ...
            mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_LINK_ROOT_e), &mLightInfluence.mPos);
...
BOOL daPy_lk_c::createHeap() {
    ...
    mpCLModelData = initModel(&mpCLModel, dRes_INDEX_LINK_BDL_CL_e, 0x37221222);
    tmp_modelData = initModel(&mpHandsModel, dRes_INDEX_LINK_BDL_HANDS_e, 0x37221222);
    tmp_modelData = initModel(&mpKatsuraModel, dRes_INDEX_LINK_BDL_KATSURA_e, 0x37221222);
    tmp_modelData = initModel(&mpYamuModel, dRes_INDEX_LINK_BDL_YAMU_e, 0x37221222);
    tmp_modelData = initModel(&mpPringModel, dRes_INDEX_LINK_BDL_PRING_e, 0x37221222);
    tmp_modelData = initModel(&mpSwgripaModel, dRes_INDEX_LINK_BDL_SWGRIPA_e, 0x37221222);
    
    tmp_modelData = initModel(&mpSwgripmsModel, dRes_INDEX_LINK_BDL_SWGRIPMS_e, 0x37221222);
    bck = (J3DAnmTransform*)dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BCK_SWGRIPMSAB_e);
    ret = mSwgripmsabBckAnim.init(mpSwgripmsModel->getModelData(), bck, false, J3DFrameCtrl::EMode_LOOP);
    if (!ret) { JUT_ASSERT(VERSION_SELECT(20859, 20966, 21046, 21046), FALSE); }
    mpTswgripmsabBrk = entryBrk(tmp_modelData, dRes_INDEX_LINK_BRK_TSWGRIPMSAB_e);
    mpTswgripmsBtk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_TSWGRIPMS_e);
    
    tmp_modelData = initModel(&mpPodmsModel, dRes_INDEX_LINK_BDL_PODMS_e, 0x37221222);
    
    tmp_modelData = initModel(&mpShaModel, dRes_INDEX_LINK_BDL_SHA_e, 0x37221222);
    bck = (J3DAnmTransform*)dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BCK_ATNGSHA_e);
    ...
    tmp_modelData = initModel(&mpYmsls00Model, dRes_INDEX_LINK_BDL_YMSLS00_e, 0x13000222);
    mpYmsls00Btk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_YMSLS00_e);
    
    tmp_modelData = initModel(&mpHbootsModels[0], dRes_INDEX_LINK_BDL_HBOOTS_e, 0x37221222);
    tmp_modelData = initModel(&mpHbootsModels[1], dRes_INDEX_LINK_BDL_HBOOTS_e, 0x37221222);
    
    tmp_modelData = initModel(&mpSuimenMunyaModel, dRes_INDEX_LINK_BDL_SUIMEN_MUNYA_e, 0x11001222);
    mpSuimenMunyaBtk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_SUIMEN_MUNYA_e);
    
    tmp_modelData = initModel(&mpYuchw00Model, dRes_INDEX_LINK_BDL_YUCHW00_e, 0x13000222);
    bck = (J3DAnmTransform*)dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BCK_YUCHW00_e);
    ret = mYuchw00Bck.init(tmp_modelData, bck, 1, J3DFrameCtrl::EMode_LOOP);
    if (!ret) { JUT_ASSERT(VERSION_SELECT(20926, 21033, 21113, 21113), FALSE); }
    mpYuchw00Btk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_YUCHW00_e);
    mpYuchw00Brk = entryBrk(tmp_modelData, dRes_INDEX_LINK_BRK_YUCHW00_e);
    mpYuchw00Brk->setFrame(mpYuchw00Brk->getFrameMax()-0.001f);
    
    tmp_modelData = initModel(&mpYbafo00Model, dRes_INDEX_LINK_BDL_YBAFO00_e, 0x13000222);
    mpYbafo00Btk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_YBAFO00_e);
    mpYbafo00Btk->setFrame(mpYbafo00Btk->getFrameMax()-0.001f);
    
    tmp_modelData = static_cast<J3DModelData*>(dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BDL_YAURA00_e));
    JUT_ASSERT(VERSION_SELECT(20943, 21050, 21130, 21130), tmp_modelData != NULL);
    daPy_aura_c* aura_p = mMagicArmorAuraEntries;
    for (int i = 0; i < 6; i++) {
        aura_p->setModel(mDoExt_J3DModel__create(tmp_modelData, 0x00080000, 0x11001222));
        JUT_ASSERT(VERSION_SELECT(20952, 21059, 21139, 21139), aura_p->getModel() != NULL);
        aura_p++;
    }
    mpYaura00Btk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_YAURA00_e);
    brk = (J3DAnmTevRegKey*)dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BRK_YAURA00_R_e);
    ret = mYaura00rBrk.init(tmp_modelData, brk, false, J3DFrameCtrl::EMode_LOOP);
    if (!ret) { JUT_ASSERT(VERSION_SELECT(20960, 21067, 21147, 21147), FALSE); }
    mYaura00rBrk.entryFrame(0.0f);
    
    tmp_modelData = initModel(&mpYmgcs00Model, dRes_INDEX_LINK_BDL_YMGCS00_e, 0x11001222);
    mpYmgcs00Btk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_YMGCS00_e);
    brk = (J3DAnmTevRegKey*)dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BRK_YMGCS00_MS_e);
    ret = mYmgcs00Brk.init(tmp_modelData, brk, false, J3DFrameCtrl::EMode_LOOP);
    if (!ret) { JUT_ASSERT(VERSION_SELECT(20974, 21081, 21161, 21161), FALSE); }
    mYmgcs00Brk.entryFrame(0.0f);
    
    tmp_modelData = static_cast<J3DModelData*>(dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BDL_BOMB_e));
    JUT_ASSERT(VERSION_SELECT(20982, 21089, 21169, 21169), tmp_modelData != NULL);
    mpBombBrk = entryBrk(tmp_modelData, dRes_INDEX_LINK_BRK_BOMB_e);
    
    tmp_modelData = static_cast<J3DModelData*>(dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BDL_GWP00_e));
    JUT_ASSERT(VERSION_SELECT(20989, 21096, 21176, 21176), tmp_modelData != NULL);
    mpGwp00BrkData = entryBrk(tmp_modelData, dRes_INDEX_LINK_BRK_GWP00_e);
    mpGwp00BtkData = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_GWP00_e);
    
    tmp_modelData = static_cast<J3DModelData*>(dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BDL_ARROW_e));
    JUT_ASSERT(VERSION_SELECT(20997, 21104, 21184, 21184), tmp_modelData != NULL);
    mpIceArrowBtk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_TTIPICE_e);
    
    tmp_modelData = static_cast<J3DModelData*>(dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BDL_ARROWGLITTER_e));
    JUT_ASSERT(VERSION_SELECT(21004, 21111, 21191, 21191), tmp_modelData != NULL);
    mpLightArrowBtk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_TARROWGLITTER_e);
    
    tmp_modelData = static_cast<J3DModelData*>(dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BDL_GICER00_e));
    JUT_ASSERT(VERSION_SELECT(21011, 21118, 21198, 21198), tmp_modelData != NULL);
    mpGicer00Btk = entryBtk(tmp_modelData, dRes_INDEX_LINK_BTK_GICER00_e);
    
    tmp_modelData = static_cast<J3DModelData*>(dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BDL_GICER01_e));
...
J3DModelData* daPy_lk_c::initModel(J3DModel** i_model, int i_fileIndex, u32 i_differedDlistFlag) {
    ...
    JUT_ASSERT(VERSION_SELECT(21153, 21260, 21340, 21340), tmp_modelData != NULL);
    *i_model = mDoExt_J3DModel__create(tmp_modelData, 0x00080000, i_differedDlistFlag);
    JUT_ASSERT(VERSION_SELECT(21157, 21264, 21344, 21344), *i_model != NULL);
...
J3DAnmTextureSRTKey* daPy_lk_c::entryBtk(J3DModelData* param_1, int param_2) {
    ...
    JUT_ASSERT(VERSION_SELECT(21186, 21293, 21373, 21373), btk_anm != NULL);
...
J3DAnmTevRegKey* daPy_lk_c::entryBrk(J3DModelData* param_1, int param_2) {
    ...
    JUT_ASSERT(VERSION_SELECT(21208, 21315, 21395, 21395), brk_anm != NULL);
...
void daPy_lk_c::playerInit() {
    u32 heapSize = 0xB0000;
    if (!fopAcM_entrySolidHeap(this, daPy_createHeap, heapSize)) {
        JUT_ASSERT(VERSION_SELECT(21243, 21350, 21430, 21430), FALSE);
    }
    ...
    mpCLModelData->getJointNodePointer(CL_JNT_CENTER_e)->setCallBack(daPy_jointCallback0);
    mpCLModelData->getJointNodePointer(CL_JNT_CL_PODA_e)->setCallBack(daPy_jointCallback0);
    mpCLModelData->getJointNodePointer(CL_JNT_CHEST_JNT_e)->setCallBack(daPy_jointCallback0);
    mpCLModelData->getJointNodePointer(CL_JNT_CL_LHANDA_e)->setCallBack(daPy_jointCallback0);
    mpCLModelData->getJointNodePointer(CL_JNT_CL_RHANDA_e)->setCallBack(daPy_jointCallback0);
    mpCLModelData->getJointNodePointer(CL_JNT_HATC_JNT_e)->setCallBack(daPy_jointCallback0);
    mpCLModelData->getJointNodePointer(CL_JNT_HEAD_JNT_e)->setCallBack(daPy_jointCallback0);
    ...
    int i;
    J3DMaterial* mtl = mpCLModelData->getJointNodePointer(CL_JNT_CL_EYE_e)->getMesh();
        ...
        while (mtl) {
            ...
            if (mtl->getZMode()->getCompareEnable() == 0) {
                if ((u8)mtl->getBlend()->getType() == GX_BM_BLEND) {
                    ...
                    JUT_ASSERT(VERSION_SELECT(21299, 21406, 21486, 21486), zoff_blend_cnt <= 4);
                } else {
                    mpZOffNoneShape[zoff_none_cnt++] = mtl->getShape();
                    JUT_ASSERT(VERSION_SELECT(21303, 21410, 21490, 21490), zoff_none_cnt <= 4);
                }
            } else {
                mpZOnShape[zon_cnt++] = mtl->getShape();
                JUT_ASSERT(VERSION_SELECT(21307, 21414, 21494, 21494), zon_cnt <= 4);
            }
            mtl = mtl->getNext();
        }
        mtl = mpCLModelData->getJointNodePointer(CL_JNT_CL_MAYU_e)->getMesh();
    }
    
    JUT_ASSERT(VERSION_SELECT(21314, 21421, 21501, 21501), zon_cnt == 4);
    JUT_ASSERT(VERSION_SELECT(21315, 21422, 21502, 21502), zoff_none_cnt == 4);
    JUT_ASSERT(VERSION_SELECT(21316, 21423, 21503, 21503), zoff_blend_cnt == 4);
    
    mpLhandShape = mpCLModelData->getJointNodePointer(CL_JNT_CL_LHANDA_e)->getMesh()->getShape();
    mpRhandShape = mpCLModelData->getJointNodePointer(CL_JNT_CL_RHANDA_e)->getMesh()->getShape();
    J3DModelData* handsModelData = mpHandsModel->getModelData();
    for (u16 jnt_no = HANDS_JNT_CL_LHANDB_e; jnt_no < HANDS_JNT_CL_RHANDE_e + 1; jnt_no++) {
    ...
    ResTIMG* blur_img = (ResTIMG*)dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BTI_BLUR_e);
    ...
    ResTIMG* tmp_tex = (ResTIMG*)dComIfG_getObjectRes("Always", dRes_INDEX_ALWAYS_I4_BALL128B_e);
    JUT_ASSERT(VERSION_SELECT(21500, 21607, 21687, 21687), tmp_tex != NULL);
    ...
    ResTIMG* linktex_casual = (ResTIMG*)dComIfG_getObjectRes(l_arcName, dRes_INDEX_LINK_BTI_LINKTEXBCI4_e);
...
cPhs_State daPy_lk_c::makeBgWait() {
    ...
    if (
        ...
        || (
            ...
            fopAcM_GetName(dComIfG_Bgsp()->GetActorPointer(mAcch.m_gnd)) == fpcNm_TBOX_e
        )
#endif
    ) {
        return cPhs_INIT_e;
    ...
    if (((fopAcM_GetParam(this) & 0x80) != 0)) {
        ...
        if (m352E != 0) {
            if (!dComIfG_Bgsp()->ChkMoveBG(mAcch.m_gnd) ||
                fopAcM_GetName(dComIfG_Bgsp()->GetActorPointer(mAcch.m_gnd)) != fpcNm_OBJ_IKADA_e)
            {
                return cPhs_INIT_e;
            }
            current.pos.y = mAcch.GetGroundH();
           ...
           current.pos.x < 400.0f && current.pos.x > -400.0f))))
    {
...
#if VERSION > VERSION_DEMO
                cXyz local_38(current.pos.x, current.pos.y + 1500.0f, current.pos.z);
                fpc_ProcID uVar8 = fopAcM_create(fpcNm_TORNADO_e, 2, &local_38, fopAcM_GetRoomNo(this));
        ...
        if (mEventIdx == 0xFF) {
            ...
            if (changeSwimProc()) {
                if (startMode == 1) {
                    mNormalSpeed = 0.5f * m_HIO->mSwim.m.field_0xC;
                }
                speedF = mNormalSpeed;
                procSwimMove_init(0);
            } else if (startMode == 1) {
                mNormalSpeed = m_HIO->mMove.m.field_0x2C * m_HIO->mMove.m.field_0x18;
                speedF = mNormalSpeed;
                procMove_init();
            } else if (sceneMode == 2) {
                m34E8 = shape_angle.y;
                ...
                if (mNormalSpeed > m_HIO->mMove.m.field_0x18) {
                    mNormalSpeed = m_HIO->mMove.m.field_0x18;
                }
                speedF = mNormalSpeed;
                procMove_init();
            }
        ...
        cXyz local_44(current.pos.x + 200.0f * cM_ssin(shape_angle.y), 150.0f + current.pos.y,
        ...
        fpc_ProcID fVar11 = fopAcM_createChild(fpcNm_FM_e, fopAcM_GetID(this), 0, &current.pos,
    ...
    if (sceneMode == 4) {
        cXyz local_50;
        mDoMtx_multVecZero(mpCLModel->getAnmMtx(CL_JNT_LINK_ROOT_e), &local_50);
...
void daPy_lk_c::setSeAnime(const daPy_anmHeap_c* anmHeap, J3DFrameCtrl* frameCtrl) {
    if (reinterpret_cast<JUTDataFileHeader*>(anmHeap->m_buffer)->mSeAnmOffset == -1) {
        ...
        JUT_ASSERT(VERSION_SELECT(22108, 22228, 22318, 22318), data_size < l_sanm_buffer_size);
        
        cLib_memCpy(m_sanm_buffer, (u8*)(anmHeap->m_buffer) + fileHeader->mSeAnmOffset, data_size);
        mpSeAnmFrameCtrl = frameCtrl;
...
void daPy_lk_c::initSeAnime() {
    if (mpSeAnmFrameCtrl) {
        ...
        mJAIZelAnime.initActorAnimSound(m_sanm_buffer, mpSeAnmFrameCtrl->getRate() >= 0.0f ? 1 : -1, loop);
    }
...
BOOL daPy_lk_c::setActAnimeUpper(u16 bckIdx, daPy_UPPER upperIdx, f32 rate, f32 start, s16 end, f32 i_morf) {
    ...
    if (i_morf >= 0.0f) {
        m_old_fdata->initOldFrameMorf(i_morf, CL_JNT_BODY_CHN_e, CL_JNT_WAIST_CHN_e);
    }
...
BOOL daPy_lk_c::resetActAnimeUpper(daPy_UPPER upperIdx, f32 i_morf) {
    if (checkDashDamageAnime()) {
        mDamageWaitTimer = m_HIO->mDam.m.field_0x0;
    }
